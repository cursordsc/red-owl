...
function payWage(player, pay, faction, tax)
	...
	if interest ~= 0 then
		dbExec(mysql:getConnection(), "INSERT INTO wiretransfers (`from`, `to`, `amount`, `reason`, `type`) VALUES (-57, " .. dbid .. ", " .. interest .. ", 'BANKINTEREST', 6)" )
	end
...
function payAllWages(isForcePayday)
	...
	for _, veh in pairs(getElementsByType("vehicle")) do
		if isElement(veh) then
			...
			if vehid >0 and faction <= 0 and owner > 0 then
				...
				local active, reason = exports['vehicle']:isActive(veh)
				if not active and not exports['vehicle']:isProtected(veh) then
	...
	for _, property in pairs( getElementsByType( "interior" ) ) do
		...
		local active, reason = exports['interiors']:isActive(property)
		if not active and not exports['interiors']:isProtected(property) then
			if exports['interiors']:unownProperty(intid, "Forcesold by Inactivity Scanner. Reason: "..reason) then
				local account = exports["cache"]:getAccountFromCharacterId(owner) or {id = 0, username="No-one"}
				local characterName = exports["cache"]:getCharacterNameFromID(owner) or "No-one"
...
function doPayDayPlayer(value, isForcePayday)
	...
	if ((logged==1) and (timeinserver>=58) and (getPlayerIdleTime(value) < 600000)) or isForcePayday then
		...
		if getElementData(value, "referrer") and getElementData(value, "referrer") > 0 and getElementData(value, "hoursplayed") == 50 then
			if dbExec(mysql:getConnection(), "UPDATE `accounts` SET `credits`=`credits`+10 WHERE `id`='"..getElementData(value, "referrer").."'" ) then
				dbExec(mysql:getConnection(), "INSERT INTO `don_purchases` SET `name`='"..exports["global"]:toSQL("Referring reward - Your friend '"..getElementData(value, "account:username").."' who has reached 50 hoursplayed on character '"..exports["global"]:getPlayerName(value).."'").."', `cost`=10, `account`='"..getElementData(value, "referrer").."'" )
...
function loadWelfare( )
	local result = dbPoll(dbQuery(mysql:getConnection(), "SELECT value FROM settings WHERE name = 'welfare'"), -1)
	if result then
