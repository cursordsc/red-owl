...
function getLanguageByName( language )
	for i = 1, call( getResourceFromName( languages ), "getLanguageCount" ) do
		if language:lower() == call( getResourceFromName( languages ), "getLanguageName", i ):lower() then
			return i
		end
	end
	return false
		...
		if not targetPlayerName or not language then
			...
			elseif getElementData( targetPlayer, "loggedin" ) ~= 1 then
				...
				if not lang then
					...
					local langname = call( getResourceFromName( languages ), "getLanguageName", lang )
					local success, reason = call( getResourceFromName( languages ), "learnLanguage", targetPlayer, lang, false, skill )
					if success then
		...
		if not targetPlayerName or not language then
			...
			elseif getElementData( targetPlayer, "loggedin" ) ~= 1 then
				...
				if not lang then
					...
					local langname = call( getResourceFromName( languages ), "getLanguageName", lang )
					if call( getResourceFromName( languages ), "removeLanguage", targetPlayer, lang ) then
