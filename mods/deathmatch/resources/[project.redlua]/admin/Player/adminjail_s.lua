	...
	if (exports["integration"]:isPlayerTrialAdmin(thePlayer)) then
		...
		if not (who) or not (minutes) or not (...) or (minutes<1) then
			...
			if (targetPlayer) then
				...
				local res = dbExec(mysql:getConnection(),'INSERT INTO adminhistory (user_char, user, admin, action, duration, reason) VALUES ("' .. (getPlayerName(targetPlayer)) .. '",' .. (tostring(getElementData(targetPlayer, "account:id") or 0)) .. ',' .. (tostring(getElementData(thePlayer, "account:id") or 0)) .. ',0,"' .. (( minutes == 999 and 0 or minutes )) .. '","' .. (reason) .. '")' )
	...
	if(isElement(jailedPlayer)) then
		...
		if (timeServed) then
			...
			if (timeLeft<=0) and not (getElementData(jailedPlayer, "pd.jailtime")) then
				...
				setElementPosition(jailedPlayer, -1571.9970703125, 720.5107421875, 7.2057056427002)
