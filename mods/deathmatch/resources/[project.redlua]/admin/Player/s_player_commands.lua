			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					if (restrain==0) then
						...
						--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncunun kelepçesini çözdü.", "RED:LUA Scripting")
						exports['items']:deleteAll(47, getElementData( targetPlayer, "dbid" ))
		...
		if not (targetPlayer) then
			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					local any = false
					local masks = exports['items']:getMasks()
		...
		if not (targetPlayer) then
			...
			if targetPlayer then
				...
				elseif (logged==1) then
					for i = 115, 116 do
						while exports['items']:takeItem(targetPlayer, i) do
						end
					end
					local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
					...
					--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncuya DISARM attı!", "RED:LUA Scripting")
	...
	if (exports["integration"]:isPlayerTrialAdmin(thePlayer)) then
		if not (targetPlayer) then
			...
			if targetPlayer then
				...
				local timer = setTimer(kickPlayer, 1000, 1, targetPlayer, getRootElement(), " tarafından yeniden bağlanmaya zorlandın "..tostring(adminTitle) .. " " .. adminName ..".")
				addEventHandler("onPlayerQuit", targetPlayer, function( ) killTimer( timer ) end)

				--redirectPlayer ( targetPlayer, "185.171.1.104", 22003 )
				local arnisaip = "185.171.1.104:22003"
				local arnisasifre = "arnisa159"

				redirectPlayer(targetPlayer, gettok(arnisaip,1,":"), tonumber(gettok(arnisaip,2,":")), password)
				--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncuya RECONNECT attırdı!", "RED:LUA Scripting")
			...
			if targetPlayer then
				...
				elseif (logged==1) then
					...
					if count > 0 then
						...
						--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncuya "..getWeaponNameFromID(weaponID).." verdi!", "RED:LUA Scripting")
					end
					if fails > 0 then
			...
			if targetPlayer then
				...
				elseif (logged==1) then
					...
					if count > 0 then
						...
						--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncuya "..getWeaponNameFromID(weaponID).." mermisi verdi!", "RED:LUA Scripting")
					end
					if fails > 0 then
	...
	for id = 0, 45, 1 do
		if id ~= 19 and id ~= 20 and id ~=21 then
			...
			if tmp == "disable" then
				...
				setWeaponProperty( id , "std", "maximum_clip_ammo", 1)
			end
		end
	end
end
...
function givePlayerItem(thePlayer, commandName, targetPlayer, itemID, ...)
	if (exports["integration"]:isPlayerTrialAdmin(thePlayer) or exports["integration"]:isPlayerScripter(thePlayer)) then
		if not (itemID) or not (...) or not (targetPlayer) then
			...
			if itemID == 114 and exports['shop']:getDisabledUpgrades()[tonumber(itemValue)] then
				outputChatBox("This item is temporarily disabled.", thePlayer, 255, 0, 0)
				return false
			end
			local targetPlayer, targetPlayerName = exports["global"]:findPlayerByPartialNick(thePlayer, targetPlayer)

			local preventSpawn = exports["items"]:getItemPreventSpawn(itemID, itemValue)
			...
			if targetPlayer then
				...
				elseif (logged==0) then
					outputChatBox("Player is not logged in.", thePlayer, 255, 0, 0)
				elseif (logged==1) then
					local name = call( getResourceFromName( "items" ), "getItemName", itemID, itemValue )
				...
				--	outputChatBox("Not possible to use this item with /giveitem, sorry.", thePlayer, 255, 0, 0)
					...
					if itemID > 0 and name and name ~= "?" then
						...
						if success then
							...
							if element then
								exports['items']:npcUseItem(element, itemID)
							else
	...
	if exports["integration"]:isPlayerEGO(thePlayer) then
		if not (price) or not (...) or not tonumber(price) or not (tonumber(price) > 0) or not (quantity) or not (tonumber(quantity) > 0) then
			...
			if success then
				...
				--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncuya "..quantity.." ID'li itemdan "..itemValue.." tane verdi!", "RED:LUA Scripting")
		...
		if not tonumber(health) or not (targetPlayer) then
			...
			if targetPlayer then
				...
				elseif not setElementHealth(targetPlayer, tonumber(health)) then
					...
					--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncunun canını "..health.." yaptı!", "RED:LUA Scripting")
				end
			end
		end
	end
...
function adminHeal(thePlayer, commandName, targetPlayer)
	if (exports["global"]:isStaffOnDuty(thePlayer)) then
		...
		if targetPlayer then
			...
			--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncunun canını fulledi!", "RED:LUA Scripting")
		end
	end
end
					...
					if (tostring(type(tonumber(armor))) == "number") then
						...
						elseif (targetPlayerFaction ~= 1) or (targetPlayerFaction ~= 15) or (targetPlayerFaction ~= 59) then
							if (exports["integration"]:isPlayerAdmin(thePlayer)) then
								...
								--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncuya "..tostring(armor).." zırh verdi!", "RED:LUA Scripting")
							else
			...
			if targetPlayer then
				...
				elseif tostring(type(tonumber(skinID))) == "number" and tonumber(skinID) ~= 0 then
					...
					if not (skin) and tonumber(oldSkin) ~= tonumber(skin) then
						...
						outputChatBox("Player " .. targetPlayerName .. " has received skin " .. skinID .. "" or "none" .. ".", thePlayer, 0, 255, 0)
						...
						--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncunun kıyafetini "..tostring(skinID).." yaptı!", "RED:LUA Scripting")
					end
				else
					outputChatBox("Invalid skin ID.", thePlayer, 255, 0, 0)
				end
...
addCommandHandler("kıyafetver", setPlayerSkinCmd, false, false)
				...
				if newName == targetPlayerName then
					...
					local dbid = getElementData(targetPlayer, "dbid")
						...
						if (name) then
							...
							--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncunun adını "..processedNewName.." yaptı!", "RED:LUA Scripting")
		...
		if not (targetPlayer) then
			...
			if targetPlayer then
				...
				elseif (targetPlayerPower > thePlayerPower) then -- Check the admin isn't slapping someone higher rank them him
					...
					--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncuya SLAP attı!", "RED:LUA Scripting")
		...
		if not (targetPlayer) or not (...) then
			...
			if targetPlayer then
				...
				if (targetPlayerPower <= thePlayerPower) then
					...
					--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncuya KİCK attı!", "RED:LUA Scripting")
				else
	...
	if exports["integration"]:isPlayerEGO(thePlayer) then
		if not (target) or not money or not tonumber(money) or not (...) then
			...
			if targetPlayer then
				...
				--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncunun parasını "..amount.."₺ yaptı!", "RED:LUA Scripting")
			end
		end
	end
end
	...
	if exports["integration"]:isPlayerEGO(thePlayer) then
		if not (target) or not money or not (...) then
			...
			if targetPlayer then
				...
				if tonumber(money) >= 5000 then
					local content = "[B]Spawned to username:[/B][INDENT]"..targetUsername.."[/INDENT][B]Character name: [/B][INDENT]"..targetCharacterName.."[/INDENT][B]Amount: [/B][INDENT]$"..amount.."[/INDENT][B]Reason: [/B][INDENT]"..reason..".[/INDENT][INDENT][/INDENT][U][I]Note: Please make a reply to this post with any additional information you may have.[/I][/U]"
					--exports["integration"]:createForumThread(thePlayer, thePlayer, 143, "/"..commandName.." $"..amount.." to ("..targetUsername..") "..targetCharacterName, content, "Please make a reply to this post with any additional information you may have")
				...
				--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncuya "..amount.."₺ verdi!", "RED:LUA Scripting")
			end
		end
	end
end
	...
	if exports["integration"]:isPlayerEGO(thePlayer) then
		if not (target) or not money or not (...) then
			...
			if targetPlayer then
				...
				--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncudan "..amount.."₺ aldı!", "RED:LUA Scripting")
			end
		end
	end
end
	...
	if (exports["integration"]:isPlayerTrialAdmin(thePlayer) or exports["integration"]:isPlayerSupporter(thePlayer)) then
		if not (target) then
			...
			if targetPlayer then
				...
				--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncuya FREEZE attı!", "RED:LUA Scripting")
			end
		end
	end
end
...
function adminDuty(thePlayer, commandName)
	if exports.integration:isPlayerTrialAdmin(thePlayer) then
		...
		if adminduty == 0 then
			...
			exports.global:sendMessageToAdmins("GÖREV: " .. username .. " isimli yetkili görev başında.")
		else
			triggerClientEvent(thePlayer, "accounts:settings:updateAccountSettings", thePlayer, "duty_admin", 0)
			exports.global:sendMessageToAdmins("GÖREV: " .. username .. " isimli yetkili görevden çıktı.")
	...
	if (exports["integration"]:isPlayerTrialAdmin(thePlayer)) then
		if not (targetPlayer) or not (...) then
			...
			if targetPlayer then
				...
				local fetchData = dbPoll(dbQuery(mysql:getConnection(), "SELECT `warns` FROM `accounts` WHERE `id`='"..(accountID).."'") , -1)
				...
				dbExec(mysql:getConnection(), "UPDATE accounts SET warns=" .. (warns) .. ", monitored = 'Was warned for " .. tostring(reason):gsub("'","''").."' WHERE id = " .. (accountID) )
				...
				if (warns>=3) then
					...
					local result = dbPoll(dbQuery(mysql:getConnection(), "SELECT SUM(`warns`) AS warns FROM `accounts` WHERE `ip`='" .. ( getPlayerIP(targetPlayer) ) .. "' OR mtaserial='" .. ( getPlayerSerial(targetPlayer) ) .."'") , -1)
					if result then
...
function resetCharacter(thePlayer, commandName, ...)
    if exports["integration"]:isPlayerAdmin(thePlayer) then
        if not (...) then
			...
			local result = dbPoll(dbQuery(mysql:getConnection(), "SELECT id, account FROM characters WHERE charactername='" .. (character) .. "'") , -1)
            ...
            if charid then
                ...
                for key, value in pairs( getElementsByType( "vehicle" ) ) do
                    if isElement( value ) then
                        if getElementData( value, "owner" ) == charid then
                            call( getResourceFromName( "items" ), "deleteAll", 3, getElementData( value, "dbid" ) )
                ...
                dbExec(mysql:getConnection(), "UPDATE `vehicles` SET `deleted`='"..(admAccID).."' WHERE owner = " .. (charid) )
				...
				function(qh)
					...
					if rows > 0 then
						for index, row in ipairs(res) do
							...
							call( getResourceFromName( "interiors" ), "publicSellProperty", thePlayer, id, false, false )
						end
					end
				end,
				mysql:getConnection(), "SELECT id FROM interiors WHERE owner = " .. (charid) .. " AND type != 2")

                exports["anticheat"]:changeProtectedElementDataEx( thePlayer, "dbid", old, false )
                -- get rid of all items, give him default items back
                dbExec(mysql:getConnection(), "DELETE FROM items WHERE type = 1 AND owner = " .. (charid) )
                -- get the skin
				local skin = 264
				local skinr = dbPoll(dbQuery(mysql:getConnection(), "SELECT skin FROM characters WHERE id = " .. (charid)) , -1)
                if skinr then
                    skin = tonumber(skinr["skin"]) or 264
                end
                dbExec(mysql:getConnection(), "INSERT INTO items (type, owner, itemID, itemValue) VALUES (1, " .. (charid) .. ", 16, " .. (skin) .. ")" )
                dbExec(mysql:getConnection(), "INSERT INTO items (type, owner, itemID, itemValue) VALUES (1, " .. (charid) .. ", 17, 1)" )
                dbExec(mysql:getConnection(), "INSERT INTO items (type, owner, itemID, itemValue) VALUES (1, " .. (charid) .. ", 18, 1)" )
                -- delete wiretransfers
                dbExec(mysql:getConnection(), "DELETE FROM wiretransfers WHERE `from` = " .. (charid) .. " OR `to` = " .. (charid) )
                -- set spawn at unity, strip off money etc
                if dbExec(mysql:getConnection(), "UPDATE characters SET `money`='500', `bankmoney`='1000' WHERE id = " .. (charid) ) then
...
function resetAccount(thePlayer, commandName, accountName)
	...
	mysql:getConnection(), "SELECT `charactername`, `money`, `bankmoney`, `hoursplayed` FROM `accounts` RIGHT JOIN `characters` ON `accounts`.`id`=`characters`.`account` WHERE `accounts`.`username`='"..tostring(accountName):gsub("'","''").."' ORDER BY `characters`.`lastlogin` DESC ")
	...
	if exports["integration"]:isPlayerTrialAdmin(thePlayer) then
		...
		if not (...) then
			...
			if charid then
				...
				dbExec(mysql:getConnection(), "UPDATE characters SET x = 1949.7724609375, y = -1793.298828125, z = 13.546875 WHERE id = " .. (charid) )
...
function vehicleLimit(admin, command, player, limit)
	 if exports["integration"]:isPlayerSeniorAdmin(admin) then
		if (not player and not limit) then
			...
			if (tplayer) then
				local query = dbPoll(dbQuery(mysql:getConnection(), "SELECT maxvehicles FROM characters WHERE id = " .. (getElementData(tplayer, "dbid"))) , -1)
				if (query) then
...
function intLimit(admin, command, player, limit)
	   if exports["integration"]:isPlayerSeniorAdmin(admin) then
		if (not player and not limit) then
			...
			local tplayer, targetPlayerName = exports["global"]:findPlayerByPartialNick(admin, player)
			if (tplayer) then
				local query = dbPoll(dbQuery(mysql:getConnection(), "SELECT `maxinteriors` FROM `characters` WHERE `id` = " .. (getElementData(tplayer, "dbid"))) , -1)
				if (query) then
