...
function openStaffManager(staffInfo1, staffTeams1, globalChangelogs1) --/ Dizzy 23.04.2022
	if canPlayerAccessStaffManager(localPlayer) then
		...
		if GUIEditor.window[1] and isElement(GUIEditor.window[1]) then
	        ...
	        GUIEditor.window[1] = guiCreateWindow(538, 144, 700, 461+yExtend, "RED:LUA Scripting - Yetkili Yönetim Arayüzü ©cursor", false)
...
addCommandHandler("yetkiver", openStaffManager, false, false)
...
function guiStaffChange()
	...
	if source == GUIEditor.combobox.admin then
		...
		if (not exports["integration"]:isPlayerSeniorAdmin(localPlayer) or myLevel < admin or myLevel < currentPos[1]) and not exports["integration"]:isPlayerLeadScripter(localPlayer) then
	...
	elseif source == GUIEditor.combobox.sup then
		...
		if not exports["integration"]:isPlayerSeniorAdmin(localPlayer) and not exports["integration"]:isPlayerSupportManager(localPlayer) then
	...
	elseif source == GUIEditor.combobox.vct then
		...
		if not exports["integration"]:isPlayerSeniorAdmin(localPlayer) and (myLevel <= vct or myLevel <= currentPos[3]) then
