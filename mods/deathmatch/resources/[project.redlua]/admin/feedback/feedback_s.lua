...
function openFeedBackDetails(staff)
	if staff then
		...
		function(qh)
			...
			if rows > 0 then
				for index, target in ipairs(res) do
					if target and target.id and tonumber(target.id) then
						...
						mysql:getConnection(), "SELECT username, rating, comment, DATE_FORMAT(date,'%b %d, %Y %h:%i %p') AS date FROM feedbacks f LEFT JOIN accounts a ON f.from_id=a.id WHERE f.staff_id="..(target.id).." ORDER BY f.id DESC")
