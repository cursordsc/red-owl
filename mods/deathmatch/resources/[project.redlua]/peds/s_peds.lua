        ...
        function(qh)
            ...
            if rows > 0 then
                for index, row in ipairs(res) do
                    ...
                    if ped then
                        exports["anticheat"]:changeProtectedElementDataEx(ped, "rpp.npc", true)
                        ...
                        local badges = {}
                        badges = exports['items']:getBadges()
                        ----outputDebugString(tostring(#badges).." badges")
                        for k,v in pairs(badges) do
                            local hasItem = exports['items']:hasItem(ped, k)
                            if hasItem then
                                local itemresult = exports['items']:npcUseItem(ped, k)
                        ...
                        --exports['items']:updateLocalGuns(ped)
                       
                        exports["anticheat"]:changeProtectedElementDataEx(ped, "rpp.npc.createdBy", row.created_by)
                        exports["anticheat"]:changeProtectedElementDataEx(ped, "rpp.npc.createdByUsername", getUserNameFromID(row.created_by))
                        ...
                        local itemresult = exports['items']:npcUseItem(ped, itemID)
                        --outputDebugString("ped itemresult = "..tostring(itemresult))
                        --]]
                    end
                end
        ...
        if not (factionType==4) then
            ...
            if foundkit then
                if isPedDead(ped) then
                    ...
                    if itemValue > 1 then
                        exports['items']:updateItemValue(player, slot, itemValue - 1)
                    else
