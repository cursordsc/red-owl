...
local white = tocolor( 43, 43, 43, 255 )
...
function drawFriends( )
	...
	local line = scroll_y
	for _, data in ipairs( { { title = "", items = online }, { title = "", items = offline } }) do
		-- render title
		...
		for key, value in ipairs( data.items ) do
			...
			dxDrawRectangle( x, y, w, h, tocolor( 255,255,255, 200 ), true )
			...
			if exports.integration:isPlayerTrialAdmin(value.player) then
				dxDrawText( value.name, x, y, w, 15, tocolor ( 235, 63, 86, 255 ), 1, font, "left", "top", false, false, true )
			...
			if value.player then
				text = "Karakter: " .. getPlayerName( value.player ):gsub( "_", " " ) .. " [" .. getElementData( value.player, "playerid" ) .. "]"
			else
			...
			if value.message then
				...
				dxDrawText( "Bu sunucu bir harika dostum.", x + 10, y, w - 10, h, white, 1, small, "left", "top", true, true, true )
			end
