	...
	function( otherAccount )
		...
		if accountID and friends[ accountID ] and friends[ otherAccount ] then
			...
			if not found then
				...
				exports["infobox"]:addBox(client, "error", "Başarıyla " .. friends[ otherAccount ].name .. " adlı oyuncuyu arkadaşlıktan çıkardınız.")
	...
	if fromID and friends[ fromID ] and toID and friends[ toID ] then
		...
		if onFromList then
			...
			if onToList then -- the OTHER player has him on his friends list. shouldn't happen, but oh well.
				...
				exports["infobox"]:addBox(from, "success", "Başarıyla " .. (getPlayerName( to ):gsub("_", " ")) .. " adlı oyuncuyu arkadaş olarak eklediniz.")
...
function addFriendCmd( thePlayer, commandName, targetPlayer )
   ...
   if fromID and friends[ fromID ] and toID and friends[ toID ] then
      ...
      if onFromList then
         ...
         if onToList then
            dbExec(mysql:getConnection(), "INSERT INTO friends VALUES(" .. (fromID) .. ", " .. (toID) .. ")" )
			exports["infobox"]:addBox(thePlayer, "success", "Başarıyla " .. (getPlayerName( to ):gsub("_", " ")) .. " adlı oyuncu arkadaşlık isteğinizi onayladı.")
			exports["infobox"]:addBox(targetPlayer, "success", "Başarıyla " .. (getPlayerName( from ):gsub("_", " ")) .. " adlı oyuncuyla arkadaş oldunuz.")
            table.insert( friends[ fromID ], toID )
...
addEventHandler( "social:acceptFriend", root,
	function( )
		...
		if not from or not to then
			...
			sendFriends( from, fromID )
			sendFriends( to, toID )
			exports["infobox"]:addBox(from, "success", "Başarıyla " .. (getPlayerName( to ):gsub("_", " ")) .. " adlı oyuncu arkadaşlık isteğinizi onayladı.")
			exports["infobox"]:addBox(to, "success", "Başarıyla " .. (getPlayerName( from ):gsub("_", " ")) .. " adlı oyuncuyla arkadaş oldunuz.")
		end
	end
)
