		...
		if not vehicle then
			...
			if not (wMechanic) then
				...
				if not noTyres[getVehicleType(vehicle)] then
					bMechanicThree = guiCreateButton( 0.05, y, 0.9, 0.1, "Lastik Değişimi - 10 $", true, wMechanic )
				...
				--if (playerDimension > 0) then
					...
					if faction == 4 or ( tonumber( getElementData(vehicle, "job") or 0 ) == 0 and ( getElementData(vehicle, "faction") == -1 or getElementData(vehicle, "faction") == faction ) ) then
						bMechanicFour = guiCreateButton( 0.05, y, 0.9, 0.1, "Araç Boyama - 100 $", true, wMechanic )
					...
					if vehicleWithPaintjob[getElementModel(vehicle)] then
						bMechanicSix = guiCreateButton( 0.05, y, 0.9, 0.1, "Desenli Boyama - 7,500 $", true, wMechanic )
					...
					-- Replace lights
					--if vehicleWithPaintjob[getElementModel(vehicle)] then
						bMechanicNine = guiCreateButton( 0.05, y, 0.9, 0.1, "Far Rengi - 4,000 $", true, wMechanic )
					...
					if getVehicleUpgradeOnSlot(vehicle, 8) ~= 0 and faction == 4 then
						bMechanicSeven = guiCreateButton( 0.05, y, 0.9, 0.1, "NOS'u kaldır", true, wMechanic )
					...
					-- Add/Remove Tint for BTR
						if not getElementData(vehicle, "tinted") then
							bMechanicEight = guiCreateButton( 0.05, y, 0.9, 0.1, "Cam Filmi Ekle - 5,500 $", true, wMechanic )
							addEventHandler( "onClientGUIClick", bMechanicEight, addTintWindow, false)
							y = y + 0.1
						else
							bMechanicEight = guiCreateButton( 0.05, y, 0.9, 0.1, "Camı Filmi Kaldır - 2,000 $", true, wMechanic )
