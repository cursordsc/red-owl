...
function updateContactDetails(id, name, number, email, address, favo)
	...
	if dbExec(mysql:getConnection(), "UPDATE `phone_contacts` SET `entryName`='"..(name).."', `entryNumber`='"..(number).."', `entryEmail`='"..(email).."', `entryAddress`='"..(address).."', `entryFavorited`='"..(favo).."' WHERE `id`='"..(id).."' ") then
...
function deleteContact(id)
	...
	if dbExec(mysql:getConnection(), "DELETE FROM `phone_contacts` WHERE `id`='"..(id).."' ") then
