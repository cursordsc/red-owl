...
function openContactDetails(name, number, yoffset)
	if isPhoneGUICreated() then
		...
		lName_contact_details = guiCreateLabel(margin,posY,lineW,20, "Adı:", false, wPhoneMenu)
		...
		lNumber_contact_details = guiCreateLabel(margin,posY,lineW,20, "Numara:", false, wPhoneMenu)
		...
		lEmail_contact_details = guiCreateLabel(margin,posY,lineW,20, "E-Mail:", false, wPhoneMenu)
		...
		lAddress_contact_details = guiCreateLabel(margin,posY,lineW,20, "Adres:", false, wPhoneMenu)
		...
		lFavo_contact_details = guiCreateLabel(margin,posY,lineW,20, "Favori:", false, wPhoneMenu)
		fFavo_contact_details = guiCreateCheckBox ( margin+lineW,posY,20,20, "", tonumber(theContact2.entryFavorited) == 1 , false, wPhoneMenu)
		
		local bW, bH = 100, 30
		bUpdate_contact_details = guiCreateButton(margin+bW,posY,bW,bH, "Guncelle", false, wPhoneMenu)
		guiSetEnabled(bUpdate_contact_details, false)
		posY = posY + lineH
		
		bCall1_contact_details = guiCreateButton(margin,posY,bW,bH, "Ara", false, wPhoneMenu)
		...
		bDelete_contact_details = guiCreateButton(margin,posY,bW,bH, "Sil", false, wPhoneMenu)
		bCancel2_contact_details = guiCreateButton(margin+bW+2,posY,bW,bH, "Kapat", false, wPhoneMenu)
		posY = posY + bH + 2
		...
		addEventHandler("onClientGUIClick", fFavo_contact_details, function()
			if source == fFavo_contact_details then
				local textName = guiGetText(fName_contact_details)
				local textNumber = guiGetText(fNumber_contact_details)
			   	guiSetEnabled(bUpdate_contact_details, validateContactNameAndNumber(textName, textNumber, theContact2.id))
			end
		...
		addEventHandler("onClientGUIClick", bUpdate_contact_details, function()
			if source == bUpdate_contact_details then
				local textName = guiGetText(fName_contact_details)
				local textNumber = guiGetText(fNumber_contact_details)
		...
		addEventHandler("onClientGUIClick", bDelete_contact_details, function()
			if source == bDelete_contact_details then
				if isPhoneGUICreated() then
					guiSetEnabled(wPhoneMenu, false)
				end
		...
		addEventHandler("onClientGUIClick", bCall1_contact_details, function()
			if source == bCall1_contact_details then
		...
		addEventHandler("onClientGUIClick", bCancel2_contact_details, function()
			if source == bCancel2_contact_details then
				if isPhoneGUICreated() then
					closeContactDetails()
		...
		addEventHandler("onClientGUIClick", bSMS_contact_details, function()
			if source == bSMS_contact_details then
				if isPhoneGUICreated() then
					toggleOffEverything()
