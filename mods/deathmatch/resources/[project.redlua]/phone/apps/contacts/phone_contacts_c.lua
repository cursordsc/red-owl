...
function openPhoneContacts(contacts, contactListLimitFromServer)
	 ...
	 if isPhoneGUICreated() then
		...
		local colName = guiGridListAddColumn(gPhoneBook,"İsim",0.5)
		local colNumber = guiGridListAddColumn(gPhoneBook,"Numara",0.4)
		
		local row = guiGridListAddRow(gPhoneBook)
		guiGridListSetItemText(gPhoneBook, row, colName, "Yeni Kişi Kaydet", false, false)
		...
		if (contactList[phone]) then
			--outputChatBox("1") -- saniyede bir kelime ben f evet q klavyemi
			local favorites = {}
			local acquaintances = {}
		...
		addEventHandler( "onClientGUIClick", gPhoneBook,
			function( button )
				-- lan sal amkif button == "left" then
					local row, col = guiGridListGetSelectedItem( gPhoneBook )
					if row ~= -1 and col ~= -1 then
--						outputChatBox("peki neden?")
						local name = guiGridListGetItemText( source , row, colName )
						local number = guiGridListGetItemText( source , row, 2 )
						if name == "Yeni Kişi Kaydet" then
							guiNewContact()
							--outputChatBox("peki neden?")
						else
...
function updateClientContactList(action, id, name, number, email, address, favo)
	if action == "delete" then
		for i = 1, #contactList[phone] do
			if tonumber(contactList[phone][i].id) == tonumber(id) then
...
function getContactNameFromContactNumber(phoneNumber, fromPhone)
	...
	return isNumberAHotline(phoneNumber)
end
