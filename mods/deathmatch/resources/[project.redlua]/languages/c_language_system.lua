...
function displayGUI(remotelanguages, rcurrslot)
	...
	if not (wLanguages) then
		...
		wLanguages = guiCreateWindow(x, y, width, height, "Dil: " ..getLanguageName(localPlayer), false)
		setElementData(wLanguages, "libengel", true)
		...
		for i = 1, 3 do
			...
			if L then
				...
				--local imgLang = guiCreateStaticImage(0.05, 0.1+offset, 0.025, 0.025, ":social/images/flags/" .. (flags[lang] or 'tr') .. ".png", true, wLanguages)
				...
				if languages[lang] then
					...
					if currslot == i then
						guiSetText(lLangName, guiGetText(lLangName) .. " (Kullanılan)")
					else
						local bUse = guiCreateButton(0.83, 0.08+offset, 0.2, 0.05, "Seç", true, wLanguages)
				...
				if currslot ~= i then
					local bUnlearnLang = guiCreateButton(0.83, 0.14+offset, 0.2, 0.05, "Unut", true, wLanguages)
	...
	--triggerEvent("convertUI", localPlayer, wLanguages)
	--exports["lib"]:convertUI(wLanguages)
end
end
