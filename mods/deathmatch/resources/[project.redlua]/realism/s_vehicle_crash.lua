...
function seatbelt(thePlayer)
	if getPedOccupiedVehicle(thePlayer) then
		...
		if (getVehicleType(theVehicle) == "BMX" or getVehicleType(theVehicle) == "Bike") or (noBelt[getElementModel(theVehicle)] and getVehicleOccupant(theVehicle, 0) ~= thePlayer) then
			outputChatBox("Kullandığınız araçta emniyet kemeri bulunmamaktadır.!", thePlayer, 255, 0, 0)
		else
			if 	(getElementData(thePlayer, "seatbelt") == true) then
				...
				exports["global"]:sendLocalMeAction(thePlayer, "emniyet kemerinin klipsini çıkarır.")
			else
				...
				exports["global"]:sendLocalMeAction(thePlayer, "emniyet kemerinin klipsini takar.")
			end
		end
	end
end
addCommandHandler("emniyetkemeri", seatbelt)
