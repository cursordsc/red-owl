...
function loadOneElevator(elevatorID, massLoad)
	...
	exports["mysql"]:getConnection(), "SELECT rot, tprot, id, x, y, z, tpx, tpy, tpz, dimensionwithin, interiorwithin, dimension, interior, car, disabled, oneway FROM `elevators` WHERE id = " .. elevatorID)
	return true
end
	...
	if ( ( vehicle and elevatorStatus[INTERIOR_TYPE]  ~= 0 and getVehicleOccupant( vehicle ) == player ) or not vehicle ) then
		...
		if vehicle then
			...
			if interiorElement and isElement(interiorElement) and getElementType(interiorElement) == "interior" then
				...
				exports["interiormanager"]:addInteriorLogs(dbid, "ENTERED/EXITED", player)
			end
