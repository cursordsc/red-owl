...
function setFee( thePlayer, commandName, theFee )
	if not theFee or not tonumber( theFee ) then
		outputChatBox( "Kullanım: /" .. commandName .. " [Ücret]", thePlayer, 255, 194, 14, true )
	else
		...
		if entrance then
			...
			if theFee >= 0 and theFee <= 100 then
				if interiorStatus[INTERIOR_TYPE] == 1 then
					if exports.integration:isPlayerTrialAdmin( thePlayer ) or interiorStatus[INTERIOR_OWNER] == getElementData( thePlayer, "dbid" ) then
						...
						if canHazFee then
							...
							if query then
								...
								exports.anticheat:changeProtectedElementDataEx(interiorElement, "intfee", theFee, true)
								exports.anticheat:changeProtectedElementDataEx(interiorElement, "entrance", interiorEntrance, true)
								outputChatBox("► #ffffff"..interiorName.." isimli interiorun için giriş ücretini $"..exports.global:formatMoney(theFee).." olarak ayarladın.", thePlayer, 235, 199, 82, true)
							...
							outputChatBox( "Bu işletme için ücret talep ediniz. Lütfen önce bir yöneticiye danışın.", thePlayer, 255, 0, 0 )
						end
					else
						outputChatBox("► #ffffffBu iş yeri sana ait değil.", thePlayer, 235, 199, 82, true)
					end
				else
					outputChatBox("► #ffffffBurası bir iş yeri değil.", thePlayer, 235, 199, 82, true)
				end
			else
				outputChatBox("► #ffffffGiriş ücreti 0 ila 100$ arası olmalıdır.", thePlayer, 235, 199, 82, true)
			end
		else
			outputChatBox("► #ffffffBir işyerinde değilsiniz.", thePlayer, 235, 199, 82, true)
		end
	end
end
addCommandHandler( "setfee", setFee )
addCommandHandler( "girispara", setFee )
	...
	if exports.integration:isPlayerTrialAdmin( thePlayer ) then
		...
		if not houseID then
			...
			if entrance then
				...
				exports["interiormanager"]:addInteriorLogs(dbid, commandName, thePlayer)
...
function setInteriorID( thePlayer, commandName, interiorID )
	if exports.integration:isPlayerAdminIII( thePlayer )  then
		local interiors = exports["official_interiors"].getInteriorsList()
		...
		elseif not interiors[interiorID] then
			...
			if exit then
				...
				if query then
					...
					exports["interiormanager"]:addInteriorLogs(dbid, commandName.." "..interiorID, thePlayer)
		...
		if not cost then
			...
			if exit then
				...
				if query then
					...
					exports["interiormanager"]:addInteriorLogs(dbid, commandName.." "..tostring(cost), thePlayer)
			...
			if exit then
				if type ~= interiorType then
					...
					if query then
						...
						exports["interiormanager"]:addInteriorLogs(dbid, commandName.." "..tostring(type), thePlayer)
...
function getInteriorID( thePlayer, commandName )
	...
	local interiors = exports["official_interiors"].getInteriorsList() --/MAXIME
		...
		if not id then
			...
			if entrance then
				...
				if interiorStatus[INTERIOR_DISABLED] then
					...
					exports["interiormanager"]:addInteriorLogs(dbid, commandName.." on", thePlayer)
				else
					...
					exports["interiormanager"]:addInteriorLogs(dbid, commandName.." off", thePlayer)
				end
				realReloadInterior(dbid)
...
function reloadInterior(thePlayer, commandName, interiorID)
	if (exports.integration:isPlayerAdminIII(thePlayer)) then
		if not interiorID then
			...
			if dbid ~= 0 then			
				realReloadInterior(dbid)
				outputChatBox("#575757liblua:#f9f9f9 Interior veritabanında yeniden yükleniyor.. #" .. dbid, thePlayer, 0, 255, 0, true)
			else
				if reloadOneInterior(tonumber(interiorID), false) then
					outputChatBox("#575757liblua:#f9f9f9 #" .. tonumber(interiorID).." ID'li interior veritabanında başarıyla yüklendi.", thePlayer, 0, 255, 0)
				end
			end
		end
	end
	...
	if exports.integration:isPlayerAdminIII( thePlayer ) then
		if not showLoadingProgressTimer then	
			...
			if not houseID then
				...
				local protected, details = isProtected(interiorElement)
				if protected then
					outputChatBox("Bu mülkiyet korunuyor, bitmesine kalan süre: "..details, thePlayer, 255, 0,0)
					return false
				end
				local active, details2 = isActive(interiorElement)
				if active and getElementData(thePlayer, "confirm:delint") ~= houseID then
					outputChatBox("Şu anda bir interior silmek üzeresin.", thePlayer)
					outputChatBox("İşlemini onaylamak için tekrardan /"..commandName.." "..houseID.." yazmalısın.", thePlayer)
				...
				if dbid ~= 0 then
					...
					if (query) then
						...
						outputChatBox("#575757liblua:#f9f9f9 Sildiğin interioru geri almak için '/restoreint " .. dbid .. "' yazmalısın.", thePlayer, 255, 194, 14,true )
						exports.logs:dbLog(thePlayer, 4, { "in"..tostring(dbid) } , "DELETEINTERIOR "..dbid)
						if hiddenAdmin == 0 then
							exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." ".. getPlayerName(thePlayer):gsub("_", " ") .. " isimli yetkili #"..dbid.." ID'li interioru sildi.")
						else
							exports.global:sendMessageToAdmins("[INTERIOR]: Gizli bir yetkili #"..dbid.." ID'li interioru sildi.")
						end
						...
						outputChatBox("[DELINT] Veritabanı hatası!", thePlayer, 255, 0, 0)
						return false
					end
				else
					return false
							...
							if rows > 0 then
								for index, row in ipairs(res) do
									...
									if (query) then
										...
										exports["interiormanager"]:addInteriorLogs(houseID, commandName, thePlayer)
...
function createInterior(thePlayer, commandName, interiorId, inttype, cost, ...)
	if exports.integration:isPlayerAdminIII( thePlayer ) then
		...
		if (not (interiorId) or not (inttype) or not (cost) or not (...) or ((tonumber(inttype)<0) or (tonumber(inttype)>3))) and (commandName:lower() == "addint" or commandName:lower() == "addinterior") then
			outputChatBox("KULLANIM: /" .. commandName .. " [Interior ID] [Tip] [Ücret] [İsim] [Yetkili Notu]", thePlayer, 255, 194, 14)
			outputChatBox("TİP 0: Ev", thePlayer, 255, 194, 14)
			outputChatBox("TİP 1: Şirket", thePlayer, 255, 194, 14)
			outputChatBox("TİP 2: Belediye (Satın alınamayan)", thePlayer, 255, 194, 14)
			outputChatBox("TİP 3: Kiralık", thePlayer, 255, 194, 14)
			outputChatBox("/addnewint yazarak otomatik interior oluşturabilirsiniz.", thePlayer, 255, 194, 0)
		elseif not exports.global:takeMoney(getTeamFromName("Government of Los Santos"), cost) then
			outputChatBox("► #ffffffBu mülkiyeti satın alamazsınız. (Devlet'e ait.)", thePlayer, 235, 199, 82, true)
		else
			...
			if interior then
				...
				if (query) then
					...
					outputChatBox("#575757liblua:#f9f9f9 #" .. id .. " ID'li interior başarıyla oluşturuldu.", thePlayer, 255, 194, 14, true)
					...
					if hiddenAdmin == 0 then
						exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." ".. getPlayerName(thePlayer):gsub("_", " ").. " ("..adminUsername..") isimli yetkili bir interior oluşturdu. (#"..id.." İsim: '"..name.."', Tip: "..inttype..", Fiyat: "..cost.." $)")
					else
						exports.global:sendMessageToAdmins("[INTERIOR]: Gizli bir yetkili interior oluşturdu. (#"..id.." İsim: '"..name.."', Tip: "..inttype..", Fiyat: "..cost.." $)")
					end
					
					exports["interiormanager"]:addInteriorLogs(id, commandName.." - id "..interiorId.." - price $"..cost.." - name "..name, thePlayer)
					
					return true
				else
					outputChatBox("Interior oluştururken bir hatayla karşılaştın, hatalı karakter kullanımı.", thePlayer, 255, 0, 0)
				end
			else
				outputChatBox("Bir hata oluştu. (" .. ( interiorID or "??" ) .. ").", thePlayer, 255, 0, 0)
			end
		end
	end
end
...
function updateInteriorExit(thePlayer, commandName)
	if exports.integration:isPlayerSeniorAdmin( thePlayer )  then
		...
		if (dimension==0) then
			outputChatBox("Bir interiorda değilsin.", thePlayer, 255, 0, 0)
		else
			...
			outputChatBox("► #ffffffInterior'un çıkış yerini güncellediniz.", thePlayer, 235, 199, 82, true)
...
function changeInteriorName( thePlayer, commandName, ...)
	if (exports.integration:isPlayerAdminIII(thePlayer)) then 
		...
		if not (...) then 
			outputChatBox("► #ffffffKullanım: /"..commandName.." [Yeni Mülkiyet İsmi]", thePlayer, 235, 199, 82, true)
		elseif (dimension==0) then
			...
			outputChatBox("► #ffffffMülkiyet adı "..name.." olarak değiştirildi.", thePlayer, 235, 199, 82, true)
			...
			if hiddenAdmin == 0 then
				...
				exports.global:sendMessageToAdmins("[INTERIOR]: Gizli bir yetkili #"..id.." ID'li interiorun ismini '"..name.."' olarak değiştirdi.")
			end
			
			
			exports["interiormanager"]:addInteriorLogs(id, commandName.." "..name, thePlayer)
...
function forceSellProperty(thePlayer, commandName, intID)
	if exports.integration:isPlayerAdminIII(thePlayer)  then
		if not intID or not tonumber(intID) or (tonumber(intID)%1 ~= 0) or (tonumber(intID) <= 0) then
			outputChatBox("► #ffffffMülkiyeti satılığa çıkarmanızı sağlar. /forcesell [Interior ID]", thePlayer, 235, 199, 82, true)
			return
		end
		local possibleInteriors = exports.pool:getPoolElementsByType('interior')
		...
		elseif interiorStatus[INTERIOR_OWNER] < 1 and interiorStatus[INTERIOR_FACTION] < 1 then
			...
			exports["interiormanager"]:addInteriorLogs(intID, commandName, thePlayer)
