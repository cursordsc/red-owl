function sitOnChair(x, y, z, rz, chair, offset)
	...
	exports.global:sendLocalMeAction(source, "yavaşça kendini arkasındaki sandalyeye bırakır.")
...
function standUp(chair)
	...
	exports.global:sendLocalMeAction(client, "yavaşça doğrulur.")
		...
		for k, v in pairs(haxChairs) do
			if v.d == getElementDimension(client) and v.i == getElementInterior(client) then
				if same(v.x, x) and same(v.y, y) and same(v.z, z) then
					outputChatBox("#575757liblua:#f9f9f9 O koltukta şu an bir başkası oturuyor.", client, 255, 0, 0, true)
					return
				end
			end
		end
