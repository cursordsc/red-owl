...
function installKeypad(thePad, theInterior)
	--outputDebugString(getElementType(theInterior))
	if setElementData( theInterior, "keypad_lock", getElementData(thePad, "id"), true) then
		if dbExec(mysql:getConnection(), "UPDATE `interiors` SET `keypad_lock`='"..getElementData(thePad, "id").."' WHERE `id`='"..getElementData(theInterior, "dbid").."' ") then
...
function uninstallKeypad(thePad, theInterior)
	if getElementData(theInterior, "status")[3] then
	...
	if theInterior and isElement(theInterior) and getElementData(theInterior, "keypad_lock") then
		...
		if count <= 0 then
			...
			dbExec(mysql:getConnection(), "UPDATE `interiors` SET `keypad_lock`=NULL, `keypad_lock_pw`=NULL, `keypad_lock_auto`=NULL WHERE `id`='"..getElementData(theInterior, "dbid").."' ")
		end
	end
...
function registerNewPasscode(intID, passcode)
	if dbExec(mysql:getConnection(), "UPDATE `interiors` SET `keypad_lock_pw`='"..passcode.."' WHERE `id`='"..intID.."' ") then
		triggerClientEvent(source, "keypadRecieveResponseFromServer", source, "registerNewPasscode - ok")
	else
		triggerClientEvent(source, "keypadRecieveResponseFromServer", source, "registerNewPasscode - failed")
	end
end
...
function togKeypadAutoLock(theInt)
	--findPadElementFromIntID(getElementData(theInt, "dbid"))
	...
	if currentState then
		if removeElementData(theInt, "keypad_lock_auto") and dbExec(mysql:getConnection(), "UPDATE `interiors` SET `keypad_lock_auto`=NULL WHERE `id`='"..intID.."' ") then
			triggerClientEvent(source, "keypadRecieveResponseFromServer", source, "togKeypadAutoLock - off")
		else
			allDone = false
		end
	else
		if setElementData(theInt, "keypad_lock_auto", not currentState) and dbExec(mysql:getConnection(), "UPDATE `interiors` SET `keypad_lock_auto`='1' WHERE `id`='"..intID.."' ") then
...
function resourceStop()
	local theResource = getResourceFromName(tostring("items"))
	if theResource then
