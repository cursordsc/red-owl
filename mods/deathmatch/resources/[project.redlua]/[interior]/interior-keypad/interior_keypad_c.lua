...
function openKeypadInterface(thePad)
	...
	isOwner = nil
	setElementData(localPlayer, "pressedAutoLock", nil)
	...
	if not thePad or not isElement(thePad) or not isElement(thePad) then
		...
		if not currentInt then
			...
			if getElementData(currentInt, "keypad_lock") and (getElementData(currentInt, "status")[1] == 0 or getElementData(currentInt, "status")[1] == 1 or getElementData(currentInt, "status")[1] == 3) then
			...
			isIntLocked = getElementData(currentInt, "status")[3]
			isOwner = tonumber(getElementData(currentInt, "status")[4]) == getElementData(localPlayer, "dbid")
		end
	end
	--[[
	errorCode = "out of order"
	...
	showCursor(true)
	exports["items"]:playSoundInvOpen()
			...
			if errorCode == "logged in" and isOwner then
				if #findPadElementFromIntID(getElementData(currentInt, "dbid")) > 1 then
					if not getElementData(localPlayer, "pressedAutoLock") then
						...
						setElementData(localPlayer, "pressedAutoLock", true)
					else
						setElementData(localPlayer, "pressedAutoLock", nil)
						togKeypad(false)
						playSoundBtn("processing", thePad)
					...
					if not currentInt or not isElement(currentInt) then
						...
						setTimer(function()
							if setElementData(currentInt, "keypad_lock_pw", encryptPW, true) then
								triggerServerEvent("registerNewPasscode", localPlayer, getElementData(currentInt, "dbid"), encryptPW)
							else
								togKeypad(true)
								playSoundBtn("warning", thePad)
								passcode = "____"
...
function closeKeypadInterface()
	if GUIEditor_Image[1] and isElement(GUIEditor_Image[1]) then
		...
		showCursor(false)
		exports["items"]:playSoundInvClose()
...
function playSyncedSound(code, thePad)
	if code == "doorLockSound" or code == "doorUnlockSound" then
		local sound = playSound3D(":interiors/"..code..".mp3", thePad[1], thePad[2], thePad[3])
