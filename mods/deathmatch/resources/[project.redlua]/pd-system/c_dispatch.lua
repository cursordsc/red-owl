...
function buildDispatchGUI()
	...
	local status = getElementData(localPlayer, "dispatch:status") or "Görevde Değil"
	local callsign
	local availability

	if status ~= "Görevde Değil" then
		callsign = getElementData(localPlayer, "dispatch:callsign") or "Seçilmemiş"
		availability = getElementData(localPlayer, "dispatch:availability") or "Seçilmemiş"
	else
		callsign = "Görevde Değil"
		availability = "Görevde Değil"
	end
	...
	GUIEditor.window[1] = guiCreateWindow(x, y, width, height, "Durum Bildirge Sistemi", false)
	guiWindowSetSizable(GUIEditor.window[1], false)

	-- Header Information
	GUIEditor.label[1] = guiCreateLabel(0.03, 0.07, 0.2, 0.07, "Görevli Memur:", true, GUIEditor.window[1])
	GUIEditor.label[7] = guiCreateLabel(0.24, 0.07, 0.20, 0.07, processedName, true, GUIEditor.window[1])

	GUIEditor.label[2] = guiCreateLabel(0.03, 0.14, 0.19, 0.07, "Zaman:", true, GUIEditor.window[1])
	...
	GUIEditor.label[4] = guiCreateLabel(0.02, 0.33, 0.13, 0.07, "Durum:", true, GUIEditor.window[1])
	GUIEditor.label[5] = guiCreateLabel(0.02, 0.41, 0.13, 0.07, "Ekip:", true, GUIEditor.window[1])
	GUIEditor.label[6] = guiCreateLabel(0.02, 0.5, 0.13, 0.07, "Mevcut Görev:", true, GUIEditor.window[1])
	GUIEditor.combobox[1] = guiCreateComboBox(0.15, 0.32, 0.35, 0.19, status, true, GUIEditor.window[1])
	guiComboBoxAddItem(GUIEditor.combobox[1], "Görevde Değil")
	guiComboBoxAddItem(GUIEditor.combobox[1], "Görevde")
	...
	guiComboBoxAddItem(GUIEditor.combobox[3], "Devriye")
	guiComboBoxAddItem(GUIEditor.combobox[3], "Kontrol")
	guiComboBoxAddItem(GUIEditor.combobox[3], "Görev Dışı")
	guiComboBoxAddItem(GUIEditor.combobox[3], "Eğitim")
	guiComboBoxAddItem(GUIEditor.combobox[3], "Trafik Denetimi")
	guiComboBoxAddItem(GUIEditor.combobox[3], "Kovalamaca")
	guiComboBoxAddItem(GUIEditor.combobox[3], "Operasyon")
	...
	GUIEditor.button[4] = guiCreateButton(0.01, 0.89, 0.98, 0.1, "Kapat", true, GUIEditor.window[1])
	...
	addEventHandler("onClientGUIComboBoxAccepted", GUIEditor.combobox[1], function () -- Change status button
		...
		if newstatus == nil or string.len(newstatus) < 2 then newstatus = "Görevde Değil" end
	...
	addEventHandler("onClientGUIComboBoxAccepted", GUIEditor.combobox[2], function () -- Change callsign
		...
		if newcallsign == nil or string.len(newcallsign) < 2 then newcallsign = "Seçilmemiş" end
	...
	addEventHandler("onClientGUIComboBoxAccepted", GUIEditor.combobox[3], function () -- Change availability
		...
		if newavailability == nil or string.len(newavailability) < 2 then newavailability = "Seçilmemiş" end
...
function canUseDispatch()
    if exports["global"]:hasItem(localPlayer, 177) and getElementData(localPlayer, "faction") == 1 then
		buildDispatchGUI()
	else
		outputChatBox("Hâla daha dumanla haberleşebileceğini düşünmen güzel.", 255, 0, 0)
	end
end
...
function getData() -- This function gets the data to display in 'memo' (display all on duty people, with their info)
	...
	for key, value in ipairs(callsigns) do
		for k, v in ipairs(getPlayersInTeam(getTeamFromName("Los Santos Polis Departmanı"))) do
			local officerCallsign = getElementData(v, "dispatch:callsign") or "Seçilmemiş"
			local officerStatus = getElementData(v, "dispatch:status")
			if officerCallsign == value and officerStatus ~= nil and officerStatus ~= "Görevde Değil" then
				local streetlocation = getElementData(v, "dispatch:street") or false
				local officerAvailability = getElementData(v, "dispatch:availability") or "Seçilmemiş"
