...
function ticketPlayer(thePlayer, commandName, targetPlayerNick, amount, ...)
	...
	if (logged==1) then
		...
		if (factionID==1 or factionID==156) then
			if not (targetPlayerNick) or not (amount) or not (...) then
				...
				if targetPlayer then
					...
					if (distance<=10) then
						...
						exports["global"]:giveMoney( getTeamFromName("Los Santos Polis Departmanı"), math.ceil(tax*amount) )
...
function takeLicense(thePlayer, commandName, targetPartialNick, licenseType, hours)
	...
	if (logged==1) then
		...
		if (ftype==2) or exports["integration"]:isPlayerTrialAdmin(thePlayer) then
			if not (targetPartialNick) then
				...
				if not (licenseType) or not (hours) or hours < 0 or (hours > 10 and not exports["integration"]:isPlayerTrialAdmin(thePlayer)) then
					outputChatBox("SYNTAX: /" .. commandName .. " [Player Partial Nick] [license Type 1:Driving 2:Weapon] [Hours]", thePlayer)
				else
					...
					if targetPlayer then
						...
						if (tonumber(licenseType)==1) then
							if(tonumber(getElementData(targetPlayer, "license.car")) == 1) then
								...
								exports["infobox"]:addBox(targetPlayer, "info", name.." isimli memur ehliyetine "..hours.." saat el koydu.")
								exports["infobox"]:addBox(thePlayer, "info", "Zanlının ehliyetine "..hours.." saat el koydun.")
								exports["anticheat"]:changeProtectedElementDataEx(targetPlayer, "license.car", -hours, true)
						...
						elseif (tonumber(licenseType)==2) then
							if(tonumber(getElementData(targetPlayer, "license.gun")) == 1) then
								...
								exports["anticheat"]:changeProtectedElementDataEx(targetPlayer, "license.gun", -hours, true)
