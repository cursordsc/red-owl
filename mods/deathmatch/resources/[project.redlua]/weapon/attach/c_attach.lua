...
local bones = {
	"Kafa", "Boyun", "Gövde", "Omurga", "Sol klavikula", "Sağ klavikula",
...
local defaultWeaponPositions = {
	...
	[29] = {29, 3, 0.175, 0.24, 0.125, 180, 240, 5},
...
function saveWeaponTable()
	...
	for skinID, attachments in pairs(weaponTable) do
		...
		for i, p in pairs(attachments) do
			...
			xmlNodeSetAttribute(weaponNode, "vücut", p[2])
...
function createSelectionWindow()
	...
	local weaponAttached = guiGridListAddColumn(weaponsGridList, "Silahı Göster.", 0.9)
	addEventHandler("onClientGUIDoubleClick", weaponsGridList, doubleClickedWeapon, false)
	populateWeaponGridList()

	local help = guiCreateLabel(12, 155, 200, 10, "Silahı göstermek için çift tıklayın.", false, sWin)
	guiSetFont(help, "default-small")

	btnSelectionEdit = guiCreateButton(0, 170, 90, 20, "Ayarla", false, sWin)
...
function createEditWindow()
	...
	local sx, sy = guiGetScreenSize()
	eWin = guiCreateWindow(sx - 250, sy - 375, 225, 325, "Silah Konumlandırma", false)
	guiWindowSetSizable(eWin, false)

	guiCreateLabel(10, 26, 210, 15,
		"Ayarlanan Silah: " .. name .. " (" .. id .. ") Skin: #" .. skin, false, eWin)
	...
	local help = guiCreateLabel(10, 255, 210, 35, "X, Y ve Z ofsetleri -0,5 ile 0,5 arasında olmalıdır.\n" ..
		"Etrafa bakmak için herhangi bir zamanda 'm' tuşuna basın.", false, eWin)
	guiSetFont(help, "default-small")
...
function saveWeaponAttachment(button, state)
	if button == "left" and state == "up" then
		...
		outputChatBox("#ee6000[Amerikan] #fafafaTebrikler, başarıyla silahın konumunu ayarladınız.", 0, 255, 0, true)
	end
end
...
function resetWeaponAttachment(button, state)
	if button == "left" and state == "up" then
		...
		outputChatBox("Pozisyonu sıfırla (" .. editingWeapon .. ") " .. getWeaponNameFromID(editingWeapon) ..
			" Skin #" .. editingSkin .. " varsayılana.", 0, 255, 0)
	end
end
...
addEventHandler("onClientResourceStart", resourceRoot,
	function()
		loadWeaponTable()
		for i, model in pairs(weaponModels) do
