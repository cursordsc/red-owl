		...
		if (exports["integration"]:isPlayerTrialAdmin(thePlayer)) then
			if showToOffDutyAdmins or (getElementData(thePlayer, "duty_admin") == 1) then
				if getElementData(thePlayer, "report_panel_mod") == "2" or getElementData(thePlayer, "report_panel_mod") == "3" then
					exports["report-system"]:showToAdminPanel(message, thePlayer, 255,0,0)
				else
...
function sendLocalMeAction(thePlayer, message, fromPlayer, ignoreDeaths)
	...
	local state, affectedPlayers = sendLocalText(thePlayer, (fromPlayer and "#B899CE ★" or "✪").." " ..  string.gsub(name, "_", " ") .. ( message:sub( 1, 1 ) == "'" and "" or " " ) .. message:gsub('"([^"]-)"', '#ffffff"%1#ffffff"#ff3366'), 255, 51, 102, 30, {}, true, ignoreDeaths, true)
...
function sendLocalDoAction(thePlayer, message, ignoreDeaths)
	...
	local state, affectedPlayers = sendLocalText(thePlayer, "#7EC7AA ★ " .. message:gsub('"([^"]-)"', '#ff1234"%1#ff1234"#008080') .. "      ((" .. getPlayerName(thePlayer):gsub("_", " ") .. "))", 255, 51, 102, 30, {}, true, ignoreDeaths)
	return state, affectedPlayers
end
addEvent("sendLocalDoAction", true)
addEventHandler("sendLocalDoAction", getRootElement(), sendLocalDoAction)
