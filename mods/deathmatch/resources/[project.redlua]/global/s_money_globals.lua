...
function giveMoney(thePlayer, amount)
	...
	elseif thePlayer and isElement(thePlayer) and amount > 0 then
		...
		if getElementType(thePlayer) == "player" then
			...
			while exports['items']:takeItem(thePlayer, 134) do
				if moneyTimer[thePlayer] >= loopLimit then
					--outputDebugString("exports['items']:takeItem() failed in global:giveMoney function!")
					break
				else
...
function takeMoney(thePlayer, amount, rest)
	...
	elseif thePlayer and isElement(thePlayer) and amount > 0 then
		...
		elseif hasMoney(thePlayer, amount) then
			if getElementType(thePlayer) == "player" then
				local sql = dbExec(mysql:getConnection(), "UPDATE characters SET money = money - " .. amount .. " WHERE id = " .. getElementData( thePlayer, "dbid" ) )
				if not sql then
					return false
				end
				...
				while exports['items']:takeItem(thePlayer, 134) do
					if moneyTimer[thePlayer] >= loopLimit then
						--outputDebugString("exports['items']:takeItem() failed in global:takeMoney function!")
						break
					else
						moneyTimer[thePlayer] = moneyTimer[thePlayer] + 1
					end
