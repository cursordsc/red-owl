...
local ped = createPed(240, 2052.1953125, 2442.859375, 10.8203125)
setElementInterior(ped, 0)
setElementDimension(ped, 0)
setElementRotation(ped, 0, 0, 333, "default", true)
--setPedAnimation( ped, "FOOD", "FF_Sit_Look", -1, true, false, false )
setElementData( ped, "talk", 1, false )
setElementData( ped, "name", "Greer Reid", false )
...
function showImpoundUI(vehElementsret)
	if not wImpound then
		...
		wImpound = guiCreateWindow(x, y, width, height, "Los Santos - Vergi Dairesi", false)
		guiWindowSetSizable(wImpound, false)
		
		bClose = guiCreateButton(0.6, 0.85, 0.2, 0.1, "Kapat", true, wImpound)
		bRelease = guiCreateButton(0.825, 0.85, 0.2, 0.1, "Öde", true, wImpound)
		...
		local col = guiGridListAddColumn(gCars, "Çekilmiş Araçların", 0.7)
		...
		lCost = guiCreateLabel(0.3, 0.85, 0.2, 0.1, "Ücret: $1000", true, wImpound)
