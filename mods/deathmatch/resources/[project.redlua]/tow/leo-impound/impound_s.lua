...
local lanes = {}
local impounderCol = {
	[1] = createColCuboid ( 2069.626953125, 2400.4326171875, 10.52702999115, 23, 12.5, 7),
	[59] = createColCuboid ( 2069.626953125, 2400.4326171875, 10.52702999115, 4.4, 7, 10 ),
}
...
releasePositions = {
	...
	[4] = { 
        { 2041.6279296875, 2468.1845703125, 10.8203125, 0, 0, 0 },
        { 2046.560546875, 2467.6748046875, 10.8203125, 0, 0, 0 },
        { 2050.7705078125, 2468.2861328125, 10.8203125, 0, 0, 0 },
    }
}
...
local convos = {
	...
	['noveh'] = {
		"Aracınızı lütfen bölgeye getirin!",
	},
	['invalid_days'] = {
...
function pedSay(pedName, convoId)
	return exports["global"]:sendLocalText( source, " [Turkce] "..pedName.." says: "..(getConvoText(convoId)), 255, 255, 255, 10 )
end
...
function openImpGui(pedName)
	...
	if not exports["global"]:hasItem(source, badgeId) then
		return exports["global"]:sendLocalText( source, " [Turkce] "..pedName.." says: "..getConvoText('nobadge'), 255, 255, 255, 10 )
	end
	local found = getVehicleWithinBooth(factionId)
	if not found then
		return exports["global"]:sendLocalText( source, " [Turkce] "..pedName.." says: "..getConvoText('noveh'), 255, 255, 255, 10 )
	end
	local freeLane, used, total = getAFreeLane(factionId)
	if not freeLane then
		return exports["global"]:sendLocalText( source, " [Turkce] "..pedName.." says: "..getConvoText('The impound lot here is full, maybe release some or impound this else where?'), 255, 255, 255, 10 )
	end
	local vehName = exports["global"]:getVehicleName(found)
	local vehId = getElementData(found, "dbid")
	--exports["global"]:sendLocalText( source, " [Turkce] "..pedName.." says: Alright, let's see. Hm.. "..vehName..", right?", 255, 255, 255, 10 )
...
function leoStartImpounding(dep, vehid, laneNumber, days, fine, first, last, badge, rank, vehName, plate, vin, volations, location, other)
	...
	if not freeLane or freeLane.veh ~= "0" then
		exports["global"]:sendLocalText( source, " [Turkce] "..pedName.." says: "..getConvoText('lane_not_free_anymore'), 255, 255, 255, 10 )
	...
	if getVehicleWithinBooth(factionId) ~= veh then
		...
		exports["global"]:sendLocalText( source, " [Turkce] "..pedName.." says: "..getConvoText('one_car_at_once'), 255, 255, 255, 10 )
		return false
	end
	--unlock it
    ...
    local sql = "UPDATE vehicles SET x='" .. (freeLane.x) .. "', y='" .. (freeLane.y) .."', z='" .. (freeLane.z) 
    .. "',   	rotx='" .. (freeLane.rx) .. "', roty='" .. (freeLane.ry) .. "', rotz='" .. (freeLane.rz) 
    .. "',     	currx='" .. (freeLane.x) .. "', curry='" .. (freeLane.y) .. "', currz='" .. (freeLane.z) 
    .. "',     	currrx='" .. (freeLane.rx) .. "', currry='" .. (freeLane.ry) .. "', currrz='" .. (freeLane.rz) 
    .. "',     	interior='" .. (freeLane.int) .. "', currinterior='" .. (freeLane.int) .. "', dimension='" .. (freeLane.dim) 
    .. "',     	currdimension='" .. (freeLane.dim) .. "', Impounded="..(time.yearday).." WHERE id='" .. (vehid) 
    .. "'"
    dbExec(mysql:getConnection(), sql)
	...
	--Take free slot
	dbExec(mysql:getConnection(), "UPDATE leo_impound_lot SET veh="..vehid..", fine="..fine..", release_date="..(tonumber(days)==0 and "NULL" or ("NOW() + INTERVAL "..days.." DAY")).." WHERE lane="..freeLane.lane.." AND veh=0 AND faction="..factionId)
	...
	report = report..other.."\n"
	dbExec(mysql:getConnection(), "INSERT INTO mdc_impounds SET veh="..vehid..", content='"..(report).."', reporter='"..(first.." "..last).."' ")
...
function getAFreeLane(factionId)
	dbQuery(
		...
		mysql:getConnection(), "SELECT * FROM leo_impound_lot "..(factionId and ("WHERE faction="..factionId) or "").." ORDER BY lane")
end
...
function unimpVeh(vehid)
	...
	if veh then
		...
		if impounder == 1 or impounder == 59 then
			dbExec(mysql:getConnection(), "UPDATE vehicles SET Impounded=0 WHERE id="..vehid)
			...
			if source then
				...
				dbExec(mysql:getConnection(), "INSERT INTO mdc_impounds SET veh="..vehid..", content='"..report.."', reporter='"..exports["global"]:getPlayerName(source).."' ")
			end
            dbExec(mysql:getConnection(), "UPDATE leo_impound_lot SET veh=0, release_date=NULL, fine=0 WHERE veh="..vehid)
		...
		for i, lane in pairs(laness) do
			if tonumber(lane.veh) ~= 0 then
				...
				if not veh or not isElement(veh) or getElementData(veh, "Impounded") == 0 or getElementData(veh, "impounder") ~= factionId then
					...
					dbExec(mysql:getConnection(), "UPDATE leo_impound_lot SET veh=0, release_date=NULL, fine=0 WHERE lane="..lane.lane)
...
function getImpoundLanes(factionId)
	...
	dbQuery(
		...
		exports["mysql"]:getConnection(), "SELECT * FROM leo_impound_lot "..(factionId and ("WHERE faction="..factionId) or "").." ORDER BY lane")
end
...
function addlane(player, cmd, fid)
		...
		if dbExec(mysql:getConnection(), "INSERT INTO leo_impound_lot SET x="..x..", y="..y..", z="..z..", rx="..rx..", ry="..ry..", rz="..rz..", `int`="..int..", dim="..dim..", faction="..fid) then
...
function getLaneFromVeh(vehid)
	--local result = dbPoll(dbQuery(mysql:getConnection(), "SELECT * FROM leo_impound_lot WHERE veh="..vehid), -1)
	return dbPoll(dbQuery(mysql:getConnection(), "SELECT * FROM leo_impound_lot WHERE veh="..vehid), -1)
end
...
function openReleaseGUI(pedName)
	...
	if not empty then
		...
		return exports["global"]:sendLocalText( source, " [Turkce] "..pedName.." says: "..(getConvoText("no_car_to_release")), 255, 255, 255, 10 )
	end
end
...
function release(pedName, vehid, cost)
	...
	dbQuery(
		function(qh)
			...
			if rows > 0 then
				for index, row in ipairs(res) do
                    if not row or row.secdiff == "NULL" then
                        outputChatBox("Internal Error.", source, 255, 0, 0)
						return false
					end
					
					if row.secdiff == "seized" then
						exports["global"]:sendLocalText( source, " [Turkce] "..pedName.." says: "..(getConvoText("I'm sorry your vehicle is seized, I can not release it until further notice, you gotta contact our officiers about that.")), 255, 255, 255, 10 )
						return false
					end
					local text, sec = exports["datetime"]:formatFutureTimeInterval(tonumber(row.secdiff))
					if sec ~= 0 then
						exports["global"]:sendLocalText( source, " [Turkce] "..pedName.." says: "..(getConvoText("Well, according to the document I got here, you gotta wait ".. text.." more to release this vehicle. Sorry..")), 255, 255, 255, 10 )
						return false
					end
				
					if not exports["global"]:takeMoney(source, cost) then
						exports["global"]:sendLocalText( source, " [Turkce] "..pedName.." says: "..(getConvoText("$"..exports["global"]:formatMoney(cost).." please..")), 255, 255, 255, 10 )
						return false
					end
					...
					local state, reason = unimpVeh(vehid)
					if state then
						exports["global"]:sendLocalText( source, " [Turkce] "..pedName.." says: "..(getConvoText("Your vehicle has been released, it's out front. (( Please remember to /park your vehicle so it does not respawn back here. ))")), 255, 255, 255, 10 )
						exports['vehicle-manager']:addVehicleLogs(vehid, "LEO UNIMPOUND.", source)
						exports["logs"]:dbLog(source, 6, {  veh }, "LEO UNIMPOUNDED ")
					else
						exports["global"]:sendLocalText( source, " [Turkce] "..pedName.." says: "..(getConvoText("Opps, sorry. "..reason)), 255, 255, 255, 10 )
					end
				end
			end
		end,
