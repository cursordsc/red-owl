function showSpeedToAdmins(velocity)
	...
	exports.hud:sendMessageToAdmins("[Possible Speedhack/HandlingHack] " .. getPlayerName(client) .. ": " .. velocity .. "Mph/".. kph .." Kph")
end
...
function showDMToAdmins(kills)
	exports.hud:sendMessageToAdmins("[Possible DeathMatching] " .. getPlayerName(client) .. ": " .. kills .. " kills in <=2 Minutes.")
end
...
function scanMoneyHacks()
	...
	for key, value in ipairs(players) do
		...
		if (logged==1) then
			if not (exports.integration:isPlayerAdmin(value)) then -- Only check if its not an admin...
	...
	for key, value in ipairs(hackers) do
		...
		local adminTitle = exports.integration:getPlayerAdminTitle(thePlayer)
