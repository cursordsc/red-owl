...
function reportLazyFix(player) -- / Yigoza
...
reportTypes = {
   ...
   --{"Mapping Issue", {44, 28}, "MAP", 0, 0, 0 }, Yigoza IF YOU EVER WANT TO BRING THIS BACK, UNCOMMENT
...
function getReportInfo(row, element)
	...
	for k,v in ipairs(staff) do
		if v == 39 or v == 43 then
			...
			if vcount==0 then
				return false, "Aktif araç yetkilisi mevcut değil. UCP'den şikayet açınız. https:/www./ucp"
		...
		elseif v == 32 then
			...
			if scount==0 then
				return false, "Aktif yazılımcı mevcut değil. UCP'den şikayet açınız. https:/www./ucp"
...
function updateReportCount()
	...
	for k, v in ipairs(getElementsByType("player")) do
		...
		open[v] = 0
		handled[v] = 0
		if exports.integration:isPlayerTrialAdmin(v) and getElementData(v, "loggedin") == 1 then
			local alreadyTold = {}
...
function sortReports(showMessage) -- Yigoza
	-- reports[slot] = { }
	...
	if originalReportID then
		...
		if report and reports[report] then
			...
			if (isElement(theAdmin)) then
					outputChatBox(" ["..abrv.." #" .. report .."] Oyuncu " .. getPlayerName(source) .. " oyundan ayrıldı.", theAdmin, 255, 126, 0)--200, 240, 120)
			else
							...
							for key, value in ipairs(getElementsByType("players")) do
								if getElementData(value, "loggedin") == 1 then
									if not alreadyTold[value] then
										outputChatBox(" ["..abrv.." #" .. report .."] Oyuncu " .. getPlayerName(source) .. " oyundan ayrıldı.", value, 255, 126, 0)
...
function handleReport(reportedPlayer, reportedReason, reportType)
	...
	for _, usergroup in ipairs(staff) do
		if string.find(SUPPORTER, usergroup) then -- Supporters
			...
			exports["infobox"]:addBox(source, "success", "Başarıyla rapor gönderdiniz. (Sıra ID: #"..tostring(slot)..") /er yazarak raporu kapatabilirsiniz.")
		...
		if not (id) then
			...
			if not (reports[id]) then
				...
				if (reportHandler) then
						...
						for k, usergroup in ipairs(staff) do
							if string.find(auxiliaryTeams, usergroup) then
								for key, value in ipairs(getElementsByType("player")) do
									if getElementData(value, "loggedin") == 1 then
										if not alreadyTold[value] then
											outputChatBox("[#" .. id .. "]#f9f9f9 "..adminTitle.." ("..adminUsername..") adlı yetkili #" .. id .. " ID'li raporu reddetti. -", value, r, g, b,true)
											alreadyTold[value] = true
										end
									end
								end
								...
								for key, value in ipairs(getElementsByType("player")) do
									if getElementData(value, "loggedin") == 1 then
										if not alreadyTold[value] then
											...
											local gmduty = getElementData(value, "duty_supporter")
											if (adminduty==1) or (gmduty==1) then
												outputChatBox("[#" .. id .. "]#f9f9f9 "..adminTitle.." ("..adminUsername..") adlı yetkili #" .. id .. " ID'li raporu reddetti. -", value, r, g, b,true)--200, 240, 120)
												alreadyTold[value] = true
											end
										end
									end
					...
					outputChatBox("[" .. timestring .. "]#f9f9f9 (#" .. id .. ") ID'li raporun "..adminTitle.." ".. getPlayerName(thePlayer) .. " adlı yetkili tarafından reddedildi.", reportingPlayer, r, g, b,true)--200, 240, 120)
...
function arBind()
	if exports.integration:isPlayerTrialAdmin(client) then
		exports.global:sendMessageToAdmins("AdmWarn: ".. getPlayerName(client) .. " rapor kabul etmeyi bind'den kaldır dostum.")
	end
end
			...
			if not (reports[id]) then
				...
				if (reportHandler) then
					...
					if reportingPlayer == thePlayer and not exports.integration:isPlayerScripter(thePlayer) then
						outputChatBox("[!]#FFFFFF Kendi Raporunu kabul edemezsin.",thePlayer, 255,0,0, true)
						return false
					end
	...
	if (exports.integration:isPlayerSeniorAdmin(thePlayer)) then
		if not (...) then
			...
			if targetAdmin then
				local id = tonumber(id)
				if not (reports[id]) then
					outputChatBox("[!]#FFFFFF Hatalı rapor id'si.", thePlayer, 255, 0, 0, true)
				elseif (reports[id][5] ~= thePlayer) and not (exports.integration:isPlayerAdmin(thePlayer)) then
					outputChatBox("[!]#FFFFFF Kendi raporunu kapatamazsın.", thePlayer, 255, 0, 0, true)
				else
					local reportingPlayer = reports[id][1]
					local reportedPlayer = reports[id][2]
								...
								if getElementData(value, "loggedin") == 1 then
									if not alreadyTold[value] then
										...
										if (adminduty==1) or (gmduty==1) then
											...
											updateVeri(thePlayer)
											--if exports.global:giveMoney(thePlayer, 100) then
											--outputChatBox("[!]#FFFFFFBir rapor kapattığınız için 100$ kazandınız", thePlayer, 0,255,0,true)
											--return end
											setTimer(function()
										    executeCommandHandler ( "raporum", thePlayer )
											end, 5000, 1)
...
function showUnansweredReportsGMs(thePlayer)
	if (exports.integration:isPlayerSupporter(thePlayer)) then
		if showTopRightReportBox(thePlayer) then
			...
			if count == 0 then
				outputChatBox("None.", thePlayer, 255, 228, 94)
			else
		...
		if showTopRightReportBox(thePlayer) then
			...
			for i = 1, #reports do
				...
				if report then
					...
					if staff then
						for k,v in ipairs(staff) do
							if (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) and not seenReport[i] then
