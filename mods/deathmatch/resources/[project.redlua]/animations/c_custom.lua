local anims_list = {
	...
	['custom_1'] = {'custom/custom_1.ifp', true, price = 0, name = "James_Bond Animasyon Fiziği", engine = 1, disabled = 0}, -- Gang 1
	['custom_2'] = {'custom/custom_2.ifp', true, price = 0, name = "Çete Animasyon Fiziği I", engine = 1, disabled = 0}, -- Gang 2
	['custom_3'] = {'custom/custom_3.ifp', true, price = 0, name = "Parkurcu Animasyon Fiziği", engine = 1, disabled = 0}, -- Parkour
	...
	['custom_6'] = {'custom/custom_6.ifp', true, price = 0, name = "RED:LUA Scripting Animasyon Fiziği I", engine = 1, disabled = 0}, -- Project Awesome
	--['custom_7'] = {'custom/custom_7.ifp', true, price = 0, name = "RED:LUA Scripting Kadın Yürüme Fiziği", engine = 1, disabled = 0}, -- Female Remaked
	['custom_8'] = {'custom/custom_8.ifp', true, price = 0, name = "Çete Animasyon Fiziği II", engine = 1, disabled = 0}, -- Gang 4
	['custom_9'] = {'custom/custom_9.ifp', true, price = 0, name = "RED:LUA Scripting Animasyon Fiziği II", engine = 1, disabled = 0}, -- New Animations for GTA:SA
	...
	['custom_11'] = {'custom/custom_11.ifp', true, price = 0, name = "Silah Tutuş Fiziği II", engine = 1, disabled = 0}, -- Gang 5
	['custom_12'] = {'custom/custom_12.ifp', true, price = 0, name = "Çete Animasyon Fiziği III", engine = 1, disabled = 0}, -- Gang 4
	...
	['fortnite_2'] = {'custom/fortnite_2.ifp', false, {"dnce_M_a", "dnce_M_b", "dnce_M_c", "dnce_M_d", "dnce_M_e"}, price = 0, name = "Fortnite Animasyonları", engine = 0, disabled = 0},
	['oturn'] = {'custom/seat-idle.ifp', false, {"SEAT_idle"}, price = 0, name = "Oturma Animasyonu", engine = 0, disabled = 0},
	['egil'] = {'custom/weapc.ifp', false, {"IDLE_tired"}, price = 0, name = "Eğilme Animasyonu", engine = 0, disabled = 0},
	['switch'] = {'custom/weapc.ifp', false, {"switch"}, price = 0, name = "Silah Değiştirme Animasyonu", engine = 0, disabled = 0},
...
addCommandHandler("fortnite",
	function(cmd, id)
		if not id then outputChatBox("#575757RED:LUA Scripting:#ffffff Komut Kullanımı: /fortnite (1-5)", 255, 0, 0, true) return end
...
addCommandHandler("rapping",
	function(cmd, id)
		if not id then outputChatBox("#575757RED:LUA Scripting:#ffffff Komut Kullanımı: /fortnite (1-8)", 255, 0, 0, true) return end
		local index = "rapping"
		local customtable = getElementData(localPlayer, "custom_animations") or {}
		if customtable[index] or (tonumber(anims_list[index].price) == 0) then
...
addCommandHandler("gang",
	function(cmd, id)
		if not id then outputChatBox("#575757RED:LUA Scripting:#ffffff Komut Kullanımı: /gang (1-10)", 255, 0, 0, true) return end
		local index = "gang"
		local customtable = getElementData(localPlayer, "custom_animations") or {}
		if customtable[index] or (tonumber(anims_list[index].price) == 0) then
...
addCommandHandler("custom",
	function(cmd, id)
		if not id then outputChatBox("#575757RED:LUA Scripting:#ffffff Komut Kullanımı: /custom (1-20)", 255, 0, 0, true) return end
...
addCommandHandler("kissing",
	function(cmd, id, target)
		if not id then outputChatBox("#575757RED:LUA Scripting:#ffffff Komut Kullanımı: /kissing (1-15)", 255, 0, 0, true) return end
		local index = "kissing"
		local customtable = getElementData(localPlayer, "custom_animations") or {}
		if customtable[index] or (tonumber(anims_list[index].price) == 0) then
			if (tonumber(id) == 11) then
				local targetPlayer = exports["global"]:findPlayerByPartialNick(localPlayer, target)
...
addCommandHandler("sex",
	function(cmd, id)
		if not id then outputChatBox("#575757RED:LUA Scripting:#ffffff Komut Kullanımı: /sex (1-15)", 255, 0, 0, true) return end
		local index = "sex"
		local customtable = getElementData(localPlayer, "custom_animations") or {}
		if customtable[index] or (tonumber(anims_list[index].price) == 0) then
...
addCommandHandler("blowjob",
	function(cmd, id)
		if not id then outputChatBox("#575757RED:LUA Scripting:#ffffff Komut Kullanımı: /blowjob (1-15)", 255, 0, 0, true) return end
		local index = "blowjob"
		local customtable = getElementData(localPlayer, "custom_animations") or {}
		if customtable[index] or (tonumber(anims_list[index].price) == 0) then
...
addCommandHandler("dance",
	function(cmd, id)
		if not id then outputChatBox("#575757RED:LUA Scripting:#ffffff Komut Kullanımı: /dance (1-10)", 255, 0, 0, true) return end
		local index = "dance"
		local customtable = getElementData(localPlayer, "custom_animations") or {}
		if customtable[index] or (tonumber(anims_list[index].price) == 0) then
...
local maintance = true

addCommandHandler("animasyonlar",
	function()
		if maintance and not exports["integration"]:isPlayerDeveloper(localPlayer) then
			outputChatBox("#575757RED:LUA Scripting:#ffffff Yeni animasyonlar ekleniyor, sistem güncelleniyor. Çok yakında aktif!", 255, 0, 0, true)
			return 
		end
		if getElementData(localPlayer, "loggedin") == 1 then
			if isElement(window) then destroyElement(window) end
			window = guiCreateWindow(0, 0, 549, 335, "RED:LUA Scripting - Animasyon Sistemi", false)
			...
			tab2 = guiCreateTab("Özel Animasyonlar", tab)
							...
							if row ~= -1 then
								...
								if customtable[index] or anims_list[index] then
									if (anims_list[index][2] == true) then
										...
										outputChatBox("#575757RED:LUA Scripting:#ffffff Animasyon başarıyla aktif edildi.", 0, 255, 0, true)
									else outputChatBox("#575757RED:LUA Scripting:#ffffff Animasyon fiziği olmayan paketleri komut ile kullanabilirsiniz, komutları görebilmek için listeden çift tıklayın.", 255, 0, 0, true) end
								else outputChatBox("#575757RED:LUA Scripting:#ffffff Animasyon verisi bulunamadı.", 255, 0, 0, true) end
							else outputChatBox("#575757RED:LUA Scripting:#ffffff Henüz bir animasyon seçmediniz.", 255, 0, 0, true) end
						...
						if row ~= -1 then
							...
							if customtable[index] or anims_list[index] then
								if (anims_list[index][2] == false) then
									outputChatBox("#575757RED:LUA Scripting:#ffffff "..anims_list[index].name.." animasyonunun komutları:", 0, 0, 255, true)
									...
									outputChatBox("#575757RED:LUA Scripting:#ffffff "..commands_text, 0, 0, 255, true)
								end
							end
						end
					end
