...
function displayStatus()
	for i,infotable in ipairs(textsToDraw) do
		...
		if posx and distance <= 45 and isLineOfSightClear(cx,cy,cz,px,py,pz,true,true,false,true,false,true,true, blocking) then -- change this when multiple ignored elements can be specified
		    ...
		    dxDrawRectangle(posx - (12 + (0.5 * width)), posy + 16, width + 23, 1, tocolor(136, 87, 201, 255))
			
			-- dxDrawRectangle(posx - (3 + (0.5 * width)),posy - (2 + (infotable[3] * fontHeight)),width + 5,19,tocolor(136, 87, 201))
			-- dxDrawRectangle(posx - (6 + (0.5 * width)),posy - (2 + (infotable[3] * fontHeight)),width + 11,19,tocolor(0,0,0,40))
			-- dxDrawRectangle(posx - (8 + (0.5 * width)),posy - (1 + (infotable[3] * fontHeight)),width + 15,17,tocolor(136, 87, 201))
			-- dxDrawRectangle(posx - (10 + (0.5 * width)),posy - (1 + (infotable[3] * fontHeight)),width + 19,17,tocolor(0,0,0,40))
			-- dxDrawRectangle(posx - (10 + (0.5 * width)),posy - (infotable[3] * fontHeight) + 1,width + 19,13,tocolor(136, 87, 201))
			-- dxDrawRectangle(posx - (12 + (0.5 * width)),posy - (infotable[3] * fontHeight) + 1,width + 23,13,tocolor(0,0,0,40))
			-- dxDrawRectangle(posx - (12 + (0.5 * width)),posy - (infotable[3] * fontHeight) + 4,width + 23,7,tocolor(136, 87, 201))
			...
			dxDrawText(infotable[2], posx - (0.5 * width), posy, posx - (0.5 * width), posy - (i * fontHeight), tocolor(136, 87, 201, 255), 1, "default-bold", "left", "top", false, false, false)
		end
	end
end
