...
local notificationFont = exports.fonts:getFont("FontAwesome",13)
local notificationFontBold = exports.fonts:getFont("FontAwesome",13)
...
local details = {
    ...
    ["error"] = {"Hata", "warning", "error.wav"},
    ["info"] = {"Bilgi", "information", ""},
    ["mod"] = {"Duyuru", "information", ""},
    ["models"] = {"Modeller yükleniyor..", "information", ""},
...
function addBox(type, msg, key, customDetails)
    ...
    if types[type] then
        ...
        soundFX = playSound("files/sounds/"..details[type][3])
        if isElement(soundFX) then
		    setSoundVolume(soundFX, 0.1)
        end
        table.insert(cache, 
            {
                ["key"] = key or #cache + 1,
...
function drawnBoxes()
    _sx, _sy = sx, sy
    local enabled, sx, nowY, w,h,sizable,turnable, sizeDetails, t, columns = true, 15, 300, 180, 50, 1, 1 -- BURA ASKO
    _nowY = nowY
    ...
    for k,v in ipairs(cache) do
        ...
        local boxWidth = length + 50
        dxDrawRoundedRectangle(pos[1], pos[2], boxWidth, 40, 8, tocolor(15, 15, 15, alpha[2]),false)
        nowY = nowY + between - 20 -- KUTULAR ARASI MESAFE
        dxDrawText(msg, pos[1] + 45, pos[2]+13, 0, 0, tocolor(176, 176, 176, alpha[2]), 0.75, notificationFont, 'left', 'top',false,false,true,true,false)
        ...
        dxDrawImage(pos[1] + 10, pos[2] + 7, 25, 25, 'files/notificationIcons/'..details[type][2]..'.png', 0,0,0, tocolor(r, g, b, alpha[2]),true)
