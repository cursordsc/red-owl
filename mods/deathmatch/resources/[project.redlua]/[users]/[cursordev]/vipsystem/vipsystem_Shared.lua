...
function SmallestID()
	local query = dbQuery(exports["mysql"]:getConnection(), "SELECT MIN(e1.id+1) AS nextID FROM `vipler` AS e1 LEFT JOIN `vipler` AS e2 ON e1.id +1 = e2.id WHERE e2.id IS NULL")
