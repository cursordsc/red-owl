		...
		function(qh)
			...
			if rows > 1 then 
				for _, row in ipairs(res) do
					...
					local guncelle = dbExec(mysql:getConnection(), "UPDATE characters SET money='".. money*7 .."', bankmoney='".. bankmoney*7 .."' WHERE id="..id.."")
					if guncelle then
						print("paralar güncellendi")
					else
						print("paralar yarra yedi")
					end
				end
			end
		end,
...
function loginCallback(queryHandler,source,username,password)
	...
	if rows > 0 then
		...
		setElementDataEx(source, "toplambakiye", tonumber(row['toplambakiye']), true)
		setElementDataEx(source, "cekilisdurum", tonumber(row['cekilisdurum']))
		setElementDataEx(source, "mikrofon:durum", tonumber(row['mikrofon']))
		setElementDataEx(source, "rulet:key", tonumber(row['rulet']))
		setElementDataEx(source, "vip", tonumber(row["vip"]))
		setElementDataEx(source, "arayuz", tonumber(row["arayuz"]))
		setElementDataEx(source, "rpplus", tonumber(row["rp"]))
		setElementDataEx(source, "youtuber", tonumber(row["youtuber"]))
		setElementDataEx(source, "roldurumu", tonumber(row["roldurumu"]))
		setElementDataEx(source, "pmKapatma", tonumber(row["pmKapatma"]))
		setElementDataEx(source, "RolDersVerilmeDurumu", tonumber(row["RolDersVerilmeDurumu"]))
...
function characterList(theClient)
    ...
    local accounts, character = getTableInformations() -- // Mahlukat
...
function reloadCharacters(oyuncu)
    local chars = characterList(oyuncu)
    setElementData(oyuncu, "account:characters", chars)
end
	...
	if exports["integration"]:isPlayerSeniorAdmin(thePlayer) then
		if not accountUsername or not newPass or not newPassConfirm then
			...
			elseif (string.len(newPass)<6) then
				triggerClientEvent(thePlayer, "accounts:error:window", thePlayer, "Şifreniz çok kısa. \n En az 6 harf içermelidir.")
			elseif (string.len(newPass)>=30) then
				triggerClientEvent(thePlayer, "accounts:error:window", thePlayer, "Şifreniz çok uzun. \n En fazla 30 harf içermelidir.")
			elseif (string.find(newPass, ";", 0)) or (string.find(newPass, "'", 0)) or (string.find(newPass, "@", 0)) or (string.find(newPass, ",", 0)) then
				triggerClientEvent(thePlayer, "accounts:error:window", thePlayer, "Şifreniz şunları içeremez ;,@'.")
			else
				dbQuery(
					function(qh, oyuncu)
						local res, rows, err = dbPoll(qh, 0)
						if rows > 0 then
							for index, value in ipairs(res) do
								local encryptionRule = value["salt"]
								local password = string.lower(md5(string.lower(md5(newPass))..encryptionRule))
								local query = dbExec(mysql:getConnection(), "UPDATE accounts SET password = '" .. password .. "' WHERE id = '" .. value["id"] .. "'")
								if query then
									triggerClientEvent(oyuncu, "accounts:error:window", oyuncu, accountUsername.." isimli hesabın şifresi başarıyla değiştirildi.")
								else
									triggerClientEvent(oyuncu, "accounts:error:window", oyuncu, "MySQL hatası, daha sonra tekrar deneyin.")
								end
							end
						else
							triggerClientEvent(oyuncu, "accounts:error:window", oyuncu, "Bu kullanıcı adına sahip bir hesap bulunamadı.")
						end
					end,
				{thePlayer}, mysql:getConnection(), "SELECT id, salt FROM accounts WHERE username ='"..accountUsername.."' LIMIT 1")
			end
		end
	end
end
		...
		function(qh, client, characterID, remoteAccountID, theAdmin, targetAccountName, location)
			...
			if rows > 0 then
				...
				if characterData then
					...
					local resource = getResourceFromName("vipsystem")
					if resource then
					local state = getResourceState(resource)
                        if state == "running" then
                                setElementData(client, "vipver", 0)
                                exports['vipsystem']:loadVIP(characterID)
                        end
					end					
					...
					setElementDataEx(client, "avatar", tonumber(characterData["avatar"]) or 1, true)
					setElementDataEx(client, "hafif:ruhsat", tonumber(characterData["hruhsat"]) or 0, true)
					setElementDataEx(client, "agir:ruhsat", tonumber(characterData["aruhsat"]) or 0, true)
					...
					-- ======================================================================= -->
					setElementDataEx(client, "KimlikNo", tonumber(characterData["KimlikNo"]), true)
					setElementDataEx(client, "SeriNumarasi", tonumber(characterData["SeriNumarasi"]), true)
					setElementDataEx(client, "KanGrubu", tonumber(characterData["KanGrubu"]), true)
					setElementDataEx(client, "OgrenimDurumu", tonumber(characterData["OgrenimDurumu"]), true)
					setElementDataEx(client, "OgretmenlikDurumu", tonumber(characterData["OgretmenlikDurumu"]), true)
					setElementDataEx(client, "AskerlikDurumu", tonumber(characterData["AskerlikDurumu"]), true)
					setElementDataEx(client, "rolDersiKontrol", tonumber(characterData["rolDersiKontrol"]), true)
					setElementDataEx(client, "TelsizKodu", tonumber(characterData["TelsizKodu"]), true)
					setElementDataEx(client, "TrafikPolisi", tonumber(characterData["TrafikPolisi"]), true)
					-- ======================================================================= -->
				...
				--	triggerClientEvent(client, "item:updateclient", client)
					...
					if (tonumber(characterData["blindfold"])==1) then
						...
						outputChatBox("Karakterinin gözü kapalı. eğer bu OOC bir eylem ise, F2 tuşunu kullanarak yönetici ile iletişime geç.", client, 255, 194, 15)
...
local komutlar = {"loadmodule", "tufansiker", "tufansiker2", "sfakelag", "fakelag", "mtasiker", "mtasiker2", "avradinizisikim", "sunucubizim"}

addEventHandler("onPlayerCommand",root,
    function(command)
        for k, v in ipairs(komutlar) do
        if (command == v)then
            cancelEvent()
        end
    end
...
function Characters_onCharacterChange()
	...
	setElementDataEx(client, "legitnamechange", 1)
	setPlayerName(source, "Belirsiz-"..getElementData(source, "playerid"))
	setElementDataEx(client, "legitnamechange", 0)
	
	--triggerEvent(client, "shop:removeMeFromCurrentShopUser",client, client)
	...
	dbQuery(
		function(qh, client, characterName, race, gender, skin, height, weight, age, languageselected, month, day)
			...
			if rows > 0 then
				...
				location = { 1481.287109375, -1751.7197265625, 15.4453125, 0, 0, 0, "Belediye"} -- KONUM 1
...
function emailDegistir(eposta)
	...
	elseif (string.find(eposta, ";", 0)) or (string.find(eposta, ":", 0)) or (string.find(eposta, "'", 0)) or (string.find(eposta, ",", 0)) then
		...
		if query then
			outputChatBox("#575757RED:LUA Scripting:#f9f9f9 E-posta adresiniz başarıyla değiştirildi.", source, 0, 255, 0, true)
			triggerClientEvent(source, "email:GUIClose", source)
		else
			triggerClientEvent(source, "accounts:error:window", source, "MySQL bağlantı hatası. F1 yetkili raporu açın.")
		end
	end
end
...
function beginSave()
	outputServerLog("[RED:LUA Scripting] Sunucu verileri basariyla kaydedildi!")
	for key, value in ipairs(getElementsByType("player")) do
...
function savePlayer(reason, player)
	...
	if isElement(player) then
		...
		if (logged==1 or reason=="Change Character") then
			...
			local arayuz = tonumber(getElementData(player, "arayuz"))
			local x, y, z = getElementPosition(player)
			local rot = getPedRotation(player)
			local health = getElementHealth(player)
			local armor = getPedArmor(player)
			...
			if tonumber(arayuz) then
				local arayuz = tonumber(arayuz)
			else
				local arayuz = 1
			end
			...
			zone = zone:gsub("["..table.concat(unallowedStrings).."]", "")
			
			local update = dbExec(mysql:getConnection(), "UPDATE characters SET x='" .. (x) .. "', y='" .. (y) .. "', z='" .. (z) .. "', rotation='" .. (rot) .. "', health='" .. (health) .. "', armor='" .. (armor) .. "', dimension_id='" .. (dimension) .. "', interior_id='" .. (interior) .. "', " .. (money) .. (businessprofit) .. "lastlogin=NOW(), lastarea='" .. (zone) .. "', timeinserver='" .. (timeinserver) .. "', alcohollevel='".. tostring( alcohollevel ) .."' WHERE id=" .. getElementData(player, "dbid")) -- , d_addiction='" .. mysql:escape_string(d_addiction) .. "'
			if not (update) then
				outputDebugString( "Karakter Verileri Kayit Edilmedi!" )
			end
			

			local update2 = dbExec(mysql:getConnection(), "UPDATE accounts SET lastlogin=NOW(), arayuz='" .. (arayuz) .. "' WHERE id = " .. getElementData(player,"account:id"))
			if not (update2) then
				outputDebugString( "Hesap Verileri Kayit Edilmedi!" )
			end
		end
	end
end
