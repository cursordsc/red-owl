...
function loadAccountSettings(player,id)
	if player and id then
		dbQuery(loadASettingsCallback, {player, id}, mysql:getConnection(),  "SELECT * FROM `account_settings` WHERE `id` = '"..id.."'")
	end
end
...
function loadCharacterSettings(player,id)
	if player and id then
		dbQuery(loadCSettingsCallback, {player, id}, mysql:getConnection(),  "SELECT * FROM `character_settings` WHERE `id` = '"..id.."'")
	end
end
