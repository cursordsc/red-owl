    ...
    if isElement(ped) then
		...
		elseif isElementStreamedIn(ped) then
			...
			if getElementType(element) == "player" then
				local Block, Anim = getPedAnimation(element)
				--exports["global"]:applyAnimation(element, Block, Anim, -1, false, true, false)
				exports["global"]:applyAnimation(element, "BIKEH", "BIKEh_Ride", -1, false, true, false)
			end
		else
			if getElementType(element) == "player" then
				local Block, Anim = getPedAnimation(element)
