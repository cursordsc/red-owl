...
function arackilit()
    local arac = localPlayer:getOccupiedVehicle()
    if arac then
        if (arac:isLocked()) then
            arac:setLocked(false)
			triggerServerEvent("sendLocalMeAction", localPlayer, localPlayer, "aracın kumandasına basarak aracın kilitini açar.")
        else 
            arac:setLocked(true)
			triggerServerEvent("sendLocalMeAction", localPlayer, localPlayer, "aracın kumandasına basarak aracın kilitler.")
        end
    end
end
...
function aracmotor()
    ...
    if getVehicleController(arac) == localPlayer then
        if arac:getData("enginebroke") == 1 then
            return
        end

        local motor = arac:getEngineState()

        if (motor) then
            arac:setEngineState(false)
            arac:setData("engine", 0)
            triggerServerEvent("sendLocalMeAction", localPlayer, localPlayer, "aracın anahtarını saat yönünün tersine çevirerek motoru kapatır.")
        else 
            arac:setEngineState(true)
            arac:setData("engine", 1)
            triggerServerEvent("sendLocalMeAction", localPlayer, localPlayer, "aracın anahtarını saat yönünde çevirerek motoru çalıştırır.")
        end
    end
end
...
function aracisik()
    ...
    if getVehicleController(arac) == localPlayer then
        if (arac:getOverrideLights() ~= 2) then
            arac:setOverrideLights(2)
        else
            arac:setOverrideLights(1)
        end
    end
end
...
function requestInventory(element)
	if (element) and (getElementType(element)=="vehicle") then
		...
		elseif (getElementData(vehicle, "owner") ~= getElementData(localPlayer, "dbid")) and not exports.global:hasItem(localPlayer, 3, getElementData(vehicle, "dbid")) then 
			outputChatBox("#575757[RED:LUA Scripting]: #ffffffBu işlemi yapabilmek için aracın anahtarına sahip olmalısınız.", 255, 0, 0, true)
		else
...
addEventHandler("showVehiclesPanel", root,
	function(table)
		...
		window = guiCreateWindow(0, 0, 389, 366, "İnaktif Araçlarım - RED:LUA Scripting", false)
        	...
        	function(b)
        		if (source == ok) then
        			...
        			if row ~= -1 and col ~= -1 then
        				...
        				outputChatBox("Araç başarıyla aktif edildi.", 255, 255, 255, true)
        			else
        				outputChatBox("Aktif etmek istediğiniz araç ID'yi girin.", 255, 255, 255, true)
        			end
        		end
        	end
        )
