...
local BizNoteFont18 = dxCreateFont ( "Bebas.ttf" , 16 )
		...
		if isElement(theProperty) then
			...
			if getDistanceBetweenPoints3D(cx,cy,cz,x,y,z) <= viewDistance then --Within radius viewDistance
				...
				if px and isLineOfSightClear(cx, cy, cz, x, y, z, true, false, false, true, true, false, false) then	
					...
					local len = dxGetTextWidth(toBeAdded, 1, BizNoteFont18)
					...
					if powner < 1 and pfaction < 1 and ptype ~= 2 then -- IF FOR SALE
						if ptype == 3 then -- RENTABLE
							toBeAdded = "Kiralık: "..exports.global:formatMoney(pcost).."₺\n"
							...
							toBeAdded = "Satılık: "..exports.global:formatMoney(pcost).."₺\n"
							toBeShowed = toBeShowed..toBeAdded
							local len = dxGetTextWidth(toBeAdded, 1, BizNoteFont18)
							if len > fontWidth then
								fontWidth = len
						...
						--if exports.integration:isPlayerAdmin(localPlayer) and (getElementData(localPlayer, "duty_admin") == 1) then
							local ownerName = 'Hiç Kimse'
							if powner > 0 then
							...
							toBeAdded = "Sahibi: "..(ownerName or "Yükleniyor..").."\n"
							toBeShowed = toBeShowed..toBeAdded
							local len = dxGetTextWidth(toBeAdded, 1, BizNoteFont18)
							if len > fontWidth then
								fontWidth = len
				...
				--	end
				   ...
				   --[[if exports['interior-system']:canPlayerSeeInteriorID(theProperty) then
					...
					if getElementData(localPlayer, "bgPro") ~= "0" then
						roundedRectangle(px-marg, py-marg, fontWidth+(marg*2), fontHeight+(marg*2), tocolor(0, 0, 0, 150))
					end
