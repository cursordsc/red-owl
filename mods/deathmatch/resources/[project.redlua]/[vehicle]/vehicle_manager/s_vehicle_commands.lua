	...
	if exports["integration"]:isPlayerAdmin( thePlayer ) then
		...
		if not vehID or not tonumber(vehID) or (tonumber(vehID) % 1 ~= 0 ) then
			...
			if veh then
				...
				if not vehID then
					exports["infobox"]:addBox(thePlayer, "error", "Bir araçta olmalısın. /reloadveh [Araç ID]")
					return false
				end
			end
		end
		...
		exports["infobox"]:addBox(thePlayer, "success", vehID.." ID'li aracı tekrar yükledin.")
	...
	if exports["integration"]:isPlayerTrialAdmin(thePlayer) or getElementData(thePlayer, "faction") == 4 or exports["integration"]:isPlayerSupporter(thePlayer) then
		if not targetPlayer or not exports["integration"]:isPlayerTrialAdmin(thePlayer) then
			if not (isPedInVehicle(thePlayer)) then
				...
				exports["infobox"]:addBox(thePlayer, "success", "Başarıyla aracını düz çevirdin.")
			...
			if targetPlayer then
				...
				if (logged==0) then
					exports["infobox"]:addBox(thePlayer, "error", "Bu oyuncu oyuna giriş yapmamış.")
				else
					...
					if pveh then
						...
						if getElementData(thePlayer, "hiddenadmin") == 1 then
							exports["infobox"]:addBox(targetPlayer, "success", "Gizli bir yetkili aracınızı düz çevirdi.")
						else
							exports["infobox"]:addBox(targetPlayer, "success", username.." isimli yetkili aracınızı düz çevirdi.")
						end
						outputChatBox("You unflipped " .. targetPlayerName:gsub("_"," ") .. "'s car.", thePlayer, 0, 255, 0)
						exports["infobox"]:addBox(thePlayer, "success", targetPlayerName:gsub("_"," ").." isimli oyuncunun aracını düz çevirdiniz.")

						addVehicleLogs(getElementData(pveh, "dbid"), commandName, thePlayer)
						exports["logs"]:dbLog(thePlayer, 4, { pveh, thePlayer }, command)
						--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncunun aracını çevirdi!", "RED:LUA Scripting")
					else
						exports["infobox"]:addBox(thePlayer, "error", targetPlayerName:gsub("_"," ") .." isimli oyuncu aracın içerisinde değil.")
					end
				end
			end
		end
...
function createTempVehicle(thePlayer, commandName, vehShopID)
	if exports["integration"]:isPlayerSeniorAdmin(thePlayer) then
		if not vehShopID or not tonumber(vehShopID) then
			exports["infobox"]:addBox(thePlayer, "info", "/veh [Araç ID (/vehlib)]")
			return false
		else
		...
		exports["infobox"]:addBox(thePlayer, "success", getVehicleName(veh).." isimli aracı spawnladınız. ID: (#"..dbid..")")
	else
		exports["infobox"]:addBox(thePlayer, "error", "Bu komutu kullanabilmek için A4 Lider Yetkili ve üstü olmalısın.")
	end
end
...
function gotoCar(thePlayer, commandName, id)
	if exports["integration"]:isPlayerTrialAdmin(thePlayer) or exports["integration"]:isPlayerSupporter(thePlayer) or exports["integration"]:isPlayerScripter(thePlayer) or exports["integration"]:isPlayerVehicleConsultant(thePlayer) then
		if not (id) then
			exports["infobox"]:addBox(thePlayer, "info", "Kullanım: /gotocar [Araç ID]")
		else
			...
			if theVehicle then
				...
				exports["infobox"]:addBox(thePlayer, "success", "Başarıyla aracın yanına ışınlandın.")
				--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..id.." ID'li araca ışınlandı!", "RED:LUA Scripting")
			else
				exports["infobox"]:addBox(thePlayer, "error", "Hatalı Araç ID'si girdin.")
			end
		end
	end
end
...
function getCar(thePlayer, commandName, id)
	if exports["integration"]:isPlayerTrialAdmin(thePlayer) or exports["integration"]:isPlayerSupporter(thePlayer) or exports["integration"]:isPlayerScripter(thePlayer) or exports["integration"]:isPlayerVehicleConsultant(thePlayer) then
		if not (id) then
			exports["infobox"]:addBox(thePlayer, "info", "Kullanım: /getcar [Araç ID]")
		else
			local theVehicle = exports["pool"]:getElement("vehicle", tonumber(id))
			if theVehicle then
				local r = getPedRotation(thePlayer)
				...
				exports["infobox"]:addBox(thePlayer, "success", "Başarıyla aracı yanına getirdin.")
				--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..id.." ID'li aracı yanına çekti!", "RED:LUA Scripting")
			else
				exports["infobox"]:addBox(thePlayer, "error", "Hatalı Araç ID'si girdin.")
			end
		end
...
function sendCar(thePlayer, commandName, id, toPlayer)
	if exports["integration"]:isPlayerTrialAdmin(thePlayer) or exports["integration"]:isPlayerSupporter(thePlayer) or exports["integration"]:isPlayerVehicleConsultant(thePlayer) then
		if not (id) or not (toPlayer) then
			exports["infobox"]:addBox(thePlayer, "info", "Kullanım: /sendcar [Araç ID] [Karakter Adı & ID]")
		else
			local theVehicle = exports["pool"]:getElement("vehicle", tonumber(id))
			...
			if theVehicle and targetPlayer then
				...
				exports["infobox"]:addBox(thePlayer, "success", "Aracı "..targetPlayerName.." isimli oyuncunun yanına gönderdin.")
				if getElementData(thePlayer, "hiddenadmin") == 1 then
					exports["infobox"]:addBox(targetPlayer, "success", "Gizli bir yetkili aracınızı yanınıza gönderdi.")
				else
					exports["infobox"]:addBox(targetPlayer, "success", exports["global"]:getPlayerFullIdentity(thePlayer).." isimli yetkili aracınızı yanınıza gönderdi.")
				end
				--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..id.." ID'li aracı "..getPlayerName(targetPlayer).." isimli oyuncuya gönderdi!", "RED:LUA Scripting")
			else
				exports["infobox"]:addBox(thePlayer, "error", "Hatalı Araç ID'si girdin.")
			end
		end
...
function sendPlayerToVehicle(thePlayer, commandName, toPlayer, id)
	if exports["integration"]:isPlayerTrialAdmin(thePlayer) or exports["integration"]:isPlayerSupporter(thePlayer) then
		if not (id) or not (toPlayer) then
			exports["infobox"]:addBox(thePlayer, "info", "Kullanım: /sendtoveh [Araç ID] [Karakter Adı & ID]")
		else
			local theVehicle = exports["pool"]:getElement("vehicle", tonumber(id))
			local targetPlayer, targetPlayerName = exports["global"]:findPlayerByPartialNick(thePlayer, toPlayer)
			if theVehicle then
				...
				if getElementData(thePlayer, "hiddenadmin") == 1 then
					exports["infobox"]:addBox(targetPlayer, "success", "Gizli bir yetkili seni aracının yanına gönderdi.")
				else
					exports["infobox"]:addBox(targetPlayer, "success", exports["global"]:getPlayerFullIdentity(thePlayer).." isimli yetkili aracının yanına gönderdi.")
				end
				--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncuyu "..id.." ID'li araca yoladı!", "RED:LUA Scripting")
			else
				exports["infobox"]:addBox(thePlayer, "error", "Hatalı Araç ID'si girdin.")
			end
		end
								...
								if (round(checkx, 6) == x) and (round(checky, 6) == y) then
									...
									if not getElementData(theVehicle, "carshop") then
										if isElementWithinColShape(theVehicle, wangs1Col) or isElementWithinColShape(theVehicle, wangs2Col) or isElementWithinColShape(theVehicle, wangs3Col) or isElementWithinColShape(theVehicle, bikeCol) then
											...
											call( getResourceFromName( "items" ), "deleteAll", 3, dbid )
											call( getResourceFromName( "items" ), "clearItems", theVehicle )
...
function fixPlayerVehicle(thePlayer, commandName, target)
	if (exports["integration"]:isPlayerTrialAdmin(thePlayer) or exports["integration"]:isPlayerSupporter(thePlayer) or exports["integration"]:isPlayerScripter(thePlayer) or exports["integration"]:isPlayerVehicleConsultant(thePlayer)) then
		if not (target) then
			outputChatBox("[!]#FFFFFF: /" .. commandName .. " [Partial Player Nick]", thePlayer, 255, 194, 14, true)
		else
			local username = getPlayerName(thePlayer)
			local targetPlayer, targetPlayerName = exports["global"]:findPlayerByPartialNick(thePlayer, target)
			...
			if targetPlayer then
				local logged = getElementData(targetPlayer, "loggedin")
				if (logged==0) then
					outputChatBox("[!]#FFFFFF Kişi oyunda değil hacım.", thePlayer, 255, 0, 0, true)
				else
					...
					if (veh) then
						...
						outputChatBox("(( Aracınız tamir edildi. Tamir eden kişi: " .. username:gsub("_", " ") .. " ))", targetPlayer, 255, 50, 0, true)
					else
						outputChatBox("That player is not in a vehicle.", thePlayer, 255, 0, 0)
					end
				end
...
addCommandHandler("afix", fixPlayerVehicle, false, false)
addCommandHandler("atamir", fixPlayerVehicle, false, false)
			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					if (veh) then
						...
						--exports["discord"]:connectWeb(""..getElementData(thePlayer, "account:username").." isimli yetkili "..getPlayerName(targetPlayer).." isimli oyuncunun aracına FİX attı!", "RED:LUA Scripting")
					else
						outputChatBox("That player is not in a vehicle.", thePlayer, 255, 0, 0)
					end
				end
...
function clearVehicleInventory(theVehicle)
	if theVehicle then
		local count = 0
		for key, item in pairs(exports["items"]:getItems(theVehicle)) do
...
function vergiFiyat(thePlayer)
	...
	local veh = getPedOccupiedVehicle(thePlayer)
		if not veh then
				exports["infobox"]:addBox(thePlayer, "error", "Bu komutu sadece aracın içerisinde kullanabilirsin.")
		return end
			local vehicleID = getElementData(veh, "dbid")
				...
				exports["infobox"]:addBox(thePlayer, "success", "Başarıyla aracın vergi borcunu sıfırladın.")
	end
end
...
function setVehTint(admin, command, target, status)
	if exports["integration"]:isPlayerHeadAdmin(admin) then
		if not (target) or not (status) then
...
function setVehiclePlate(thePlayer, theCommand, vehicleID, ...)
	if exports["integration"]:isPlayerHeadAdmin(thePlayer) or exports["integration"]:isPlayerLeadScripter(thePlayer) or exports["integration"]:isPlayerVehicleConsultant(thePlayer) then
	...
	if not tonumber(seat) then
		...
		if theVehicle then
			...
			if seat <= maxSeats then
				...
				if not occupant then
					if seat == 0 then
						if not getElementData(thePlayer, "license.car.cangetin") and getElementData(theVehicle, "owner") == -2 then -- Fixed your script, Dizzy. - Adams
							outputChatBox("(( This DoL Car is for the Driving Test only. ))", thePlayer, 255, 194, 14)
							return false
						end

						local job = getElementData(theVehicle, "job")
						if job ~= 0 then -- Fixed your script, Dizzy. - Adams
