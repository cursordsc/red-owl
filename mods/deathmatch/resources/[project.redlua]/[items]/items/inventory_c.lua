-- Configuration
local background_color = tocolor( 0, 0, 0, 225 )
...
local empty_color = tocolor( 200, 200, 200, 55 )
...
local active_tab_color = tocolor(0,0,0, 190) --127, 255, 127, 127 )
local logoTexture = dxCreateTexture("images/redlua.png")
...
local actionIcons =
{
	...
	[TAB_ITEMS] = { 48, tocolor( 255, 255, 255, 50 ), "Sırt Çantası" },
	[TAB_KEYS] = { -203, tocolor( 255, 255, 255, 50 ), "Anahtarlar" },
	[TAB_WEAPONS] = { -204, tocolor( 255, 255, 255, 50 ), "Silahlar" },
	
	[ACTION_DROP] = { -202, tocolor( 127, 255, 127, 63 ), "Eşyayı Bırak", "CTRL tuşuna basarak eşyanın üzerine tıkla." },
	[ACTION_SHOW] = { -201, tocolor( 127, 127, 255, 63 ), "Eşyayı Göster" },
	[ACTION_DESTROY] = { -200, tocolor( 255, 127, 127, 63 ), "Eşyayı Çöpe At", "DELETE tuşuna basarak eşyanın üzerine tıkla." },
...
local function getOverlayText( itemID, itemValue, isGrouped )
	...
	elseif itemID == 134 then -- Money / sawMTA
	...
	function( )
		...
		elseif not guiGetInputEnabled( ) and not isMTAWindowActive( ) and isCursorShowing( ) then
			...
			if show then
				...
				local y2 = y + sbox
				dxDrawRoundedRectangle( x2-20, y2/1.25, sbox, (( jrows - irows + 1 ) * sbox + spacer)*1.3, 5, background_color )
				for i = irows, jrows do
					...
					dxDrawImage( boxx+3, boxy+10, 35, 35, getImage( icon[1] ) )
					dxDrawImage( x2-7, y+7, 55, 55, logoTexture, 0, -120 )


					local boxx = x2 + spacer
                    local boxy = y2 + spacer + sbox * ( i - irows )
                    dxDrawImage( boxx+3, boxy+10, 35, 35, "images/"..icon[1]..".png", 0,0,0, i == activeTab and tocolor(65,65,65,200) or tocolor(255, 255, 255, 255))
				...
				-- actual inv
				dxDrawRoundedRectangle( x, y, columns * sbox + spacer, rows * sbox + spacer,5, background_color )
				for i = 1, columns * 5 do
					...
					if item then
						if not isMove or item[4] ~= clickItemSlot.id then
							...
							dxDrawRoundedRectangle( boxx, boxy, box, box,5, empty_color )
						...
						dxDrawText("BOŞ", boxx,boxy,boxx+box,boxy+box,tocolor(255,255,255,100),1.1,font,"center","center")					
					end
				end
			end
				...
				if clickWorldItem then
					...
					if isCursorOverInventory then
						...
						elseif hoverElement then
							...
							elseif getElementModel( hoverElement ) == 2942 and item [ 1 ] == 150 then -- ATM card into ATM/ sawMTA
								color = background_movetoelement_color
							...
							elseif getElementModel( hoverElement ) == 3761 then -- shelf - sawMTA
							...
							elseif tonumber(getElementData(hoverElement, "itemID")) == 166 and item[1] == 165 then -- Video disc into video player /sawMTA
								color = background_movetoelement_color
							else
								color = full_color
							end
					...
					if not isCursorOverInventory then
						...
						elseif hoverElement then
							...
							if getElementType( hoverElement ) == "vehicle" then
								color = background_movetoelement_color
							elseif getElementModel( hoverElement ) == 2203 and item [ 1 ] == 337 then
								color = background_movetoelement_color
						...
						elseif not clickItemSlot.group then
							...
							elseif hoverAction == ACTION_DESTROY then
								...
								elseif itemID == 134 then --Money
									outputChatBox("Parayı silemezsin.")
								else
									triggerServerEvent( "destroyItem", localPlayer, itemSlot )
								end
...
function roundedRectangle(x, y, w, h, borderColor, bgColor, postGUI)
	if (x and y and w and h) then
		...
		dxDrawRectangle(x, y, w, h, tocolor(255,255,255,0), postGUI);
