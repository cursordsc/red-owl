...
function removeOOC(text)
	if not text then
		outputChatBox("Error code 0001 occurs, please report to RED:LUA MTA Scripting Team at https:/www.RED:LUAroleplay.com/ucp")
		return ""
	end
...
function useItem(itemSlot, additional)
	...
	if itemID then
		...
		exports["oocshop"]:silahKasa(source)
		elseif (itemID==583) then
		exports["oocshop"]:premiumKasa(source)		
		...
		elseif (itemID==10) then -- red dice
			...
			exports.global:sendLocalText(source, " ✪ " .. getPlayerName(source):gsub("_", " ") .. " bir " .. (max == 6 and "" or (max .. "-yüzlü ")) .. " zar atar ve '" .. math.random( 1, max ) .."' sayısı gelir.", 255, 51, 102)
		...
		elseif (itemID==69) then -- Dictionary
			local learned = call(getResourceFromName("languages"), "learnLanguage", source, itemValue, true)
			local lang = call(getResourceFromName("languages"), "getLanguageName", itemValue)
		...
		elseif (itemID==77) then -- Card Deck
			...
			exports.global:sendLocalText(source, " ✪ " .. getPlayerName(source):gsub("_", " ") .. " bir kart çeker ve kart ".. sign[snumber] .. ( number == 1 and "n" or "" ) .. " " .. cards[number] .." gelmiştir.", 255, 51, 102)
		...
		elseif itemID == 134 then
			outputChatBox("#ebc746RED:LUA:#f9f9f9 Cüzdanında toplam " .. exports.global:formatMoney(itemValue) .. " TL paran var.", source, 255, 0, 255, true)
		...
		elseif masks[itemID] then
			...
			if itemID == 56 and not getElementData(source, "faction") == 1 then
				outputChatBox("#ebc746RED:LUA: #ffffffBu öğeyi sadece VIP'ler veya İEM üyeleri kullanabilir. VIP satın almak için ( /market )", source, 255, 0, 0, true)
				return false
			end
			local mask = getElementData(source, data[1])
		...
		elseif itemID == 580 then
			outputChatBox("RED:LUA:#f9f9f9 Tamir kiti geçici olarak devre dışı bırakılmıştır.", source, 255, 0, 0, true)
		--[[
			local itemValue = tonumber(itemValue)
			local veh = getPedOccupiedVehicle(source)
			...
			if source.vehicle then
				...
				if itemValue >= 1 then
					...
					--outputChatBox("#ebc746RED:LUA:#ffffff Aracınızı başarıyla tamir ettirdiniz.", source, 0, 255, 0, true)
				else
					takeItemFromSlot(source, itemSlot)
				end
			end
			...
			if not (getElementData(source, "Atatürk Bayrağı")) then
				...
				if texture then
					for k,v in ipairs(texture) do
						exports["item_texture"]:addTexture(bayrak, v[2], v[1])
					end
				end
				--exports.bone_attach:attachElementToBone(bayrak, source, 11, 0, 0, 0, 0, 180, 100)
				exports.bone_attach:attachElementToBone(bayrak, source, 12, 0, 0, 0, 0, 200, 30)
				...
				exports.bone_attach:detachElementFromBone(bayrak)
			...
			if badges[itemID] then
				if ( getElementData( source, badges[itemID][1] ) ) then
					...
					if pedname then
						...
						exports.global:sendLocalText(source, "* "..tostring(pedname)..", "..badges[itemID][2].." isimli şeyi çıkartır.", 255, 51, 102, 5)
					end
				else
						...
						if key ~= itemID then
							if ( getElementData ( source, badge[1] ) ) then
								...
								if pedname then
									...
									exports.global:sendLocalText(source, "* "..tostring(pedname)..", "..badge[2].." isimli şeyi çıkartır.", 255, 51, 102, 5)
								end
							end
						end
					end
...
function destroyItem(itemID)
	...
	outputChatBox("#ebc746RED:LUA:#f9f9f9 " .. itemName .. " isimli nesneyi çöp kutusuna bıraktın.", source, 255, 194, 14, true)
...
function isWatchingTV( player )
	return exports['freecam']:isPlayerFreecamEnabled( player )
end
...
function toggleBadge(source, badges, itemID , itemValue)
	if ( getElementData( source, badges[itemID][1] ) ) then
		...
		triggerEvent('sendAme',  source, "" .. badges[itemID][1] .. " kafasına giydirir." )
			...
			if getElementData(source, "spamblock") then outputChatBox("#ebc746RED:LUA:#f9f9f9 İşlem yaparken biraz yavaş olmalısın. (15 sn)", source, 255, 0, 0, true) return end
...
function christmasLotteryUseTicket(santa)
	...
	if hasItem then
		...
		if prizes[prize] then
			...
			exports.global:sendLocalMeAction(santa, "sağ elinde bulunan "..tostring(itemName).." isimli öğeyi "..tostring(playerName).." kişisine verir.")
		else
...
function istiridye(player)
	if exports.global:takeItem(player, 555) then
		local itemName = ""
		local itemID
		local randomNumber = math.random(0,9)
		exports.global:sendLocalMeAction(player, "yavaşça sağ eliyle istiridyeyi açar.")
		
		setTimer(function()
			if ((randomNumber==1) or (randomNumber==4) or (randomNumber == 5)) then
				exports.global:sendLocalDoAction(player, "Elimdeki istiridyemin içerisinden hiç bir şey bulamadım.")
			elseif ((randomNumber==2) or (randomNumber==8)) then
				itemName = "Yeşil İnci"
				itemID = 558
				exports.global:sendLocalDoAction(player, "Elimdeki istiridyenin içerisinde "..itemName.." bulunmaktadır.")
				exports.global:giveItem(player, itemID, 1)	
			...
			elseif (randomNumber==8) then
				...
				outputChatBox("RED:LUA:#f9f9f9 İstiridyenin içerisi boş çıktı.", player, 75, 75, 75, true)
			end
		end, 1000, 1)
	else
		outputChatBox("RED:LUA: #f9f9f9Bu komutu kullanabilmek için bir adet istiridyeye ihtiyacın var.", player, 75, 75, 75, true)
	end
end
