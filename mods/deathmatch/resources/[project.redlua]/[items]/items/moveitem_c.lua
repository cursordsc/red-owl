...
function hasKey( key, element )
	...
	elseif getElementData(getLocalPlayer(), "duty_admin") == 1 then
		...
		if interiorType==2 then
			if (not exports.factions:isInFactionType(getLocalPlayer(), 2) and not exports.factions:isInFactionType(getLocalPlayer(), 3)) then
				return false, false
			else
				return true, true
			end
...
local function openElementInventory( ax, ay )
	if not getElementData(localPlayer, "exclusiveGUI") then
		...
		if not elementtype then
			if getElementParent(getElementParent(source)) == getResourceRootElement(getResourceFromName("item_world")) then
		...
		wInventory = guiCreateStaticImage(ax-w/5+350, ay/2+50, w, h, ":resources/window_body.png",false)
...
function collisionFix()
	for key, value in pairs( getElementsByType( "object", getResourceRootElement( getResourceFromName("item_world") ) ) ) do
