...
function canPickup(player, item)
	...
	elseif isProtected(player, item) then
		...
		if isElement(item) then
			if exports['item_world']:can(player, "pickup", item) then
				return true
			else
				return false
			end
...
function canMove(player, item)
	...
	elseif isProtected(player, item) then
		...
		if isElement(item) then
			if exports['item_world']:can(player, "move", item) then
				return true
			else
				return false
			end
	...
	if not ammo then
		...
		if itemID == 162 then
			...
			if hoursPlayed < 9999999 then
				...
				exports.global:sendMessageToAdmins("|| RED:LUA || BUG Koruma > ".. getPlayerName(client) .. ", Yere Body Armor atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
			else
		...
		if itemID == 56 then
			...
			if hoursPlayed < 9999999 then
				outputChatBox("Bu nesneyi yere atmaniz sunucu tarafindan yasaklanmistir.", source, 255, 0, 0)
				exports.global:sendMessageToAdmins("|| RED:LUA || BUG Koruma > ".. getPlayerName(client) .. ", Yere Yüz Maskesi atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
			else
				exports.global:takeMoney(source, itemValue)
		...
		if itemID == 134 then
			...
			if hoursPlayed < 9999999 then
				...
				exports.global:sendMessageToAdmins("|| RED:LUA || BUG Koruma > ".. getPlayerName(client) .. ", Yere para atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
			else
				exports.global:takeMoney(source, itemValue)
		...
		if itemID == 254 then
			...
			--outputDebugString("Check")
			if hoursPlayed < 9999999 then
				outputChatBox("#ebc746RED:LUA: #ffffffMalesef, bu eşyayı yere atamazsınız.", source, 255, 0, 0, true)
				--outputChatBox("Yetkililere bunu yapmaya calistigin bildirildi.", source, 255, 0, 0)
				--exports.global:sendMessageToAdmins("|| RED:LUA || BUG Koruma > ".. getPlayerName(client) .. ", Yere para atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
		...
		if itemID == 116 then
			...
			if hoursPlayed < 9999999 then
				outputChatBox("Bu nesneyi yere atmaniz sunucu tarafindan yasaklanmistir.", source, 255, 0, 0)
				outputChatBox("Yetkililere bunu yapmaya calistigin bildirildi.", source, 255, 0, 0)
				exports.global:sendMessageToAdmins("|| RED:LUA || BUG Koruma > ".. getPlayerName(client) .. ", Yere mermi atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
			else
				exports.global:takeMoney(source, itemValue)
		...
		if itemID == 115 then
			...
			--exports.global:sendMessageToAdmins("|| RED:LUA || BUG Koruma > ".. getPlayerName(client) .. ", Yere silah atmaya calisti.")
			--return
			triggerClientEvent( source, "finishItemDrop", source )
		end
		...
		elseif itemID == 139 then
			...
			elseif itemID == 134 then
				if not exports.global:takeMoney(source, itemValue) then
					...
					while exports['items']:takeItem(source, 134) do
					end
			...
			elseif (itemID == 147) then -- Picture Frame
				...
				if dimension > 0 or (exports.integration:isPlayerDeveloper(source) and exports.global:isAdminOnDuty(source)) or exports.integration:isPlayerScripter(source) then
					...
					if url and texture then
						if exports['textures']:newTexture(source, url, texture) then
			...
			if insert then
				...
				local obj = exports['item_world']:createItem(id, itemID, itemValue, modelid, x, y, z + zoffset - 0.05, rx, ry, rz+rz2)
				...
				if objTexture then
					for objTexKey, objTexVal in ipairs(objTexture) do
						exports['item_texture']:addTexture(obj, objTexVal[2], objTexVal[1])
					end
				end
		...
		if tonumber(getElementData(source, "duty")) > 0 then
			outputChatBox("#ebc746RED:LUA: #ffffffSilahlarınızı görev başındayken bırakamazsınız.", source, 255, 0, 0, true)
		elseif tonumber(getElementData(source, "job")) == 4 and itemID == 41 then
			outputChatBox("#ebc746RED:LUA: #ffffffBu sprey kutusunu bırakamazsın.", source, 255, 0, 0, true)
		else
			...
			if query then
				...
				local obj = exports['item_world']:createItem(id, -itemID, ammo, modelid, x, y, z - 0.4, 75, -10, rz2)
				exports.pool:allocateElement(obj)
...
function pickupItem(object, leftammo)
	...
	-- Inventory Tooltip
	if (getResourceFromName("tooltips"))then
	...
	if not canPickup(source, object) then
		outputChatBox("#ebc746RED:LUA: #ffffffBu öğeyi alamazsınız. F2 ile bir yetkiliye başvurun.", source, 255, 0, 0, true)
	...
	if ((itemID >= 31) and itemID <= 43) or itemBannedByAltAltChecker[itemID] then
		...
		if hoursPlayedTo and hoursPlayedTo < 10 and not exports.global:isStaffOnDuty(source) then
			outputChatBox("#ebc746RED:LUA: #ffffff"..getItemName( itemID ).." yerden alabilmek için 10 saat oynaman olmalıdır.", source, 255, 0, 0, true)
			exports.anticheat:changeEld(object, "transfering", nil)
			return false
		end
		...
		if creator ~= picker then
			...
			if tonumber(accountCreator.id) == getElementData(source, "account:id") then
				outputChatBox("#ebc746RED:LUA: #ffffffVarlıkların aynı hesaptaki karakterleri veya bir oyuncunun sahip olduğu çoklu hesaplar arasında aktarılması kesinlikle yasaktır! ", source, 255, 0, 0, true)
	...
	if itemID == 115 then
		if isThisGunDuplicated(itemValue, source) then
			destroyElement(object)
			dbExec(mysql:getConnection(), "DELETE FROM worlditems WHERE id='" .. id .. "'")
			outputChatBox("#ebc746RED:LUA: #ffffffSilah çoğaltılması tespit edilerek silahınız silindi, üzgünüz.", source, 255,0,0, true)
			return false
		end
	end
	...
	if itemID == 138 then
		if not exports.integration:isPlayerAdmin(source) then
			outputChatBox("#ebc746RED:LUA: #ffffffBu öğeyi sadece bir yetkili alabilir.", source, 255, 0, 0, true)
			exports.anticheat:changeEld(object, "transfering", nil)
			return false
		end
	end
