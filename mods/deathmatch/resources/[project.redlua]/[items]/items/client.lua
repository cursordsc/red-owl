...
function showItemMenu(element, absX, absY)
	...
	if itemID == 81 or itemID == 103 then
		...
		bPickup = guiCreateButton(sx, sy, 0.9, 0.1, "Pick Item Up", true, wRightClick)
...
addCommandHandler("itemprotect",
	function(command, f)
		...
		exports.hud:sendBottomNotification(localPlayer, "Protect set to faction: " .. ( fp == -100 and "admins only" or tostring(fp)))
	end
)

local pickupTimer = nil -- RED:LUA
...
function copyClipboard()
	...
	if setClipboard(text) then
		exports.hud:sendBottomNotification(localPlayer, "Copying", "Başarıyla '" .. text .. "' kopyalandı.")
	else
		exports.hud:sendBottomNotification(localPlayer, "Copying", "Bir hata oluştu.")
	end
end
			...
			if (row==-1) or (col==-1) then
				...
				if not items[slot] then
					...
					if (desc=="A Dictionary.") then
						local res = getResourceFromName("languages")
...
--function useItem(button)
	...
	if itemSlot > 0 then -- ITEMS
		...
		elseif (itemID==6) then -- radio
            outputChatBox("RED:LUA:#f9f9f9 /tuneradio komutunu kullanmalısın.", 50, 50, 50, true)
            hideInventory()			
			return
		...
		elseif (itemID==19) then -- MP3 PLayer
			exports.vehicle_radio:showMP3GUI()
			hideInventory()
			return
		elseif (itemID==248) then -- Flashlight RED:LUA
			exports["billiard"]:billardTableShop(getLocalPlayer())	
			hideInventory()
			return				
		...
		elseif (itemID==78) then
			exports.hud:sendBottomNotification(localPlayer, itemName, "This RED:LUA Pilot License was issued for " .. itemValue .. ".")
			return
		...
		elseif (itemID==111) then
			exports.hud:sendBottomNotification(localPlayer, itemName,  "A personal GPS, shiny. Got the latest maps installed." )
			return
...
function isWatchingTV()
	return exports['freecam']:isFreecamEnabled()
end
