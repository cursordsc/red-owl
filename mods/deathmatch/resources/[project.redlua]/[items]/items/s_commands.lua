...
function writeNote(thePlayer, commandName, ...)
	...
	elseif getElementData(thePlayer, 'note-timeout') and math.abs(getElementData(thePlayer, 'note-timeout') - tick) < 5000 then
		...
		outputChatBox("#ebc746RED:LUA:#f9f9f9 Bu komutu kullanabilmek için A5+ yetkiliye ihtiyacın var.", thePlayer, 255, 0, 0, true)
	end
end
		...
		if theVehicle then
			...
			if dbid > 0 then
				if interiortype == 2 then
					...
					exports['interior_manager']:addInteriorLogs(dbid, 'changelock', thePlayer)
