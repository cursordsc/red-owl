...
local function output(from, to, itemID, itemValue, evenIfSamePlayer)
	...
	if getElementType(from) == "player" and getElementType(to) == "player" then
		...
		exports.global:sendLocalMeAction(from, "sağ elinde bulunan ".. getItemName( itemID, itemValue ).." isimli öğeyi "..getPlayerName( to ):gsub("_", " ").." kişisine verir.")
	-- player to item
	elseif getElementType(from) == "player" then
		...
		if itemID == 150 then --ATM card / MAXIME
			...
			triggerEvent('sendAme',  from, "Aracın Bagajından " .. getItemName( itemID, itemValue ) .. " inside the ".. name .." Nesnesini alır." )
		end
	-- item to player
	elseif getElementType(to) == "player" then
		...
		if itemID == 150 then --ATM card / MAXIME
			...
			triggerEvent('sendAme',  to, "Aracın Bagajından " .. getItemName( itemID, itemValue ) .. " from the ".. name .." Nesnesini alır." )
									local veh = getPedOccupiedVehicle(source)
			if veh then
			local dbid = getElementData(veh,"dbid")
			--exports["log"]:logKayit(dbid,getPlayerName( source ) , getItemName( itemID,itemValue )  ,name, "Aldi")
			end
		end
	end
	...
	if not ammo then  
		...
		if item then
			...
			if not (getElementModel( element ) == 2942) and not hasSpaceForItem( element, item[1], item[2] ) then --Except for ATM Machine / MAXIME
				...
				elseif (item[1] == 179 and getElementType(element) == "vehicle") then --vehicle texture
					...
					if(exports.global:isStaffOnDuty(source) or exports.integration:isPlayerScripter(source) or exports.global:hasItem(source, 3, tonumber(vehID)) or (getElementData(veh, "faction") > 0 and exports.factions:isPlayerInFaction(source, getElementData(veh, "faction"))) ) then
						...
						if url and texName then
							local res = exports["item_texture"]:addVehicleTexture(source, veh, texName, url)
							if res then
				...
				if getElementType( element ) == "vehicle" then
              --  exports["log"]:logKayit(getElementID(element),getPlayerName( source ) , getItemName( item[1],item[2] )  ,name,"Koydu")
                exports.logs:dbLog(source, 39, source, getPlayerName( source ) .. "->" .. name .. " #" .. getElementID(element) .. " - " .. getItemName( item[1] ) .. " - " .. item[2] )
                elseif getElementType( element ) == "player" then
                -- exports["log"]:logKayitt(getElementID(source),getPlayerName( element ) , getItemName( item[1],item[2] )  ,getPlayerName(source),"Aldı")
               -- exports["log"]:logKayit(getElementID(element),getPlayerName( source ) , getItemName( item[1],item[2] )  ,getPlayerName(element),"Verdi")
                end			
				end
		end
	else -- IF AMMO
...
function getName(element)
	...
	if getElementParent(getElementParent(element)) == getResourceRootElement(getResourceFromName("item_world")) then
