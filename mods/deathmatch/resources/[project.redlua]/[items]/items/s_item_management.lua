...
local function getID(element)
	...
	elseif getElementType(element) == "vehicle" then -- Vehicle
		return getElementData(element, "dbid")
	elseif getElementParent(getElementParent(element)) == getResourceRootElement(getResourceFromName("item_world")) then -- World Item
							...
							if query then
								...
								if itemID == 115 then -- guns
									...
									if isThisGunDuplicated(itemValue, target) then
										...
										outputChatBox("#ebc746RED:LUA: #ffffffFazla uyanık olma Query was here :P.", target, 255, 0, 0, true)
								...
								if ( (itemID >= 31) and (itemID <= 43) ) or itemBannedByAltAltChecker[itemID] then
									...
									if not exports.global:isStaffOnDuty(to) and not exports.global:isStaffOnDuty(from) then
										if hoursPlayedFrom < 10 then
											outputChatBox("#ebc746RED:LUA: #ffffff"..getItemName( itemID ).." isimli eşyayı "..getName(to).."'a transfer edebilmek için 10 saat oynaman olması gerekmektedir.", from, 255, 0, 0, true)
											return false, "Item transferi engellendi, < 10 saat"
										end
										
										if hoursPlayedTo < 10 then
											outputChatBox("#ebc746RED:LUA: #ffffff"..getItemName( itemID ).." isimli eşyayı "..getName(from).." size transfer edebilmesi için 10 saat oynaması olması gerekmektedir.", to, 255, 0, 0, true)
											return false, "Item transferi engellendi, < 10 saat"
										end
									end
								end
...
function getMaxWeight(element)
	...
	elseif getElementType( element ) == "vehicle" then
		...
		elseif isTruck( element ) then
			return tonumber(exports["jobs"]:getTruckCapacity(element)) or 120
		else
			return 20
		end
...
function deleteAll( itemID, itemValue )
	if itemID then
		...
		-- remove world items
		local objects = getElementsByType('object', getResourceRootElement(getResourceFromName('item_world')))
		if objects then
				...
				function(qh)
					...
					if rows > 0 then
						for index, row in ipairs(res) do
							Async:foreach(getElementsByType("object", getResourceRootElement(getResourceFromName("item_world"))), function(value)
								if isElement( value ) then
	...
	function(thePlayer, commandName, targetName)
		if exports.integration:isPlayerTrialAdmin(thePlayer) then
			if not (targetName) then
				return outputChatBox("|| RED:LUA || /" .. commandName .. " [Oyuncu İsmi/ID]", thePlayer, 255, 194, 14)
			end
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetName)
			if not targetPlayer then
				return outputChatBox("#ebc746RED:LUA: #ffffffKişi bulunamadı.", thePlayer, 255, 0, 0, true)
			end
			loadItems(targetPlayer, true)
			outputChatBox("#ebc746RED:LUA: #ffffffKişinin envanteri başarıyla yüklendi.", thePlayer, 0, 255, 0, true)
		end
	end
)
