...
function showItemList()
	if (exports.integration:isPlayerSeniorAdmin(localPlayer) or exports.integration:isPlayerTrial(localPlayer) or exports.integration:isPlayerScripter(localPlayer)) then
		if not (wItemsList) then
			local width, height = 800,600
			local scrWidth, scrHeight = guiGetScreenSize()
			local x = scrWidth/2 - (width/2)
			local y = scrHeight/2 - (height/2)
			wItemsList = guiCreateWindow(x, y, width, height,"Envanter Eşya Yönetim Arayüzü | © RED:LUA",false)
