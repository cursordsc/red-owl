			...
			if rows > 0 then
				for index, row in ipairs(res) do
					...
					if itemID < 0 then -- weapon
						...
						local modelid = exports['items']:getItemModel(itemID, itemValue)
						...
						local rx, ry, rz, zoffset = exports['items']:getItemRotInfo(itemID)
...
function loadWorldItems(res)
	...
	exports["mysql"]:getConnection(), "SELECT `id` FROM `worlditems`")
	setTimer(restartResource, 60000, 1, getResourceFromName("item-texture")) -- Restart item texture resource 60 seconds after all world items loading done. / Maxime
end
