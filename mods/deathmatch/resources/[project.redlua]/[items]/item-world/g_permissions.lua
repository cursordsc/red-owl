...
function canEditItemProperties(thePlayer, object)
	...
	if exports.global:hasItem(localPlayer, 4, interiorID) or exports.global:hasItem(localPlayer, 5, interiorID) or (exports.integration:isPlayerTrialAdmin(thePlayer) and exports.global:isAdminOnDuty(thePlayer)) or exports.integration:isPlayerScripter(thePlayer) then
		return true
	end
	return false
end
...
function can(player, action, element)
	...
	elseif usePerm == 8 then --query string
		...
		local badges = exports["items"]:getBadges()
					...
					if isNumeric(theItem[1]) then --if number, then treat it as a itemID
						...
						if hasItem then
							...
							if theItem[2] then
								...
								----outputDebugString(tostring(exports['items']:countItems(thePlayer, tonumber(theItem[1]), theItem[2])))
								if(tonumber(exports['items']:countItems(thePlayer, tonumber(theItem[1]), theItem[2])) > 0) then
									countOr = countOr + 1
								...
								--if exports["items"]:isBadge(theItem[1]) then
									--if(getElementData(thePlayer, badges[theItem[1]][1])) then
									--if exports["items"]:isWearingBadge(thePlayer, theItem[1]) then
									--	countOr = countOr + 1
						...
						elseif textFunction == "FL" or textFunction == "FACTIONLEADER" then --check for faction leadership
							if theItem[2] then --if a value is specified
								...
								if isNumeric(checkFaction[1]) then --if it is a number, we're checking for faction ID
									if isNumeric(checkFaction[2]) then --if faction rank is specified and is a number (valid)
										local isMember, rank, isLeader = exports.factions:isPlayerInFaction(thePlayer, tonumber(checkFaction[1]), tonumber(checkFaction[2]))
										if isMember and isLeader then
											countOr = countOr + 1
										end
									else
										local isMember, rank, isLeader = exports.factions:isPlayerInFaction(thePlayer, tonumber(checkFaction[1]))
										if isMember and isLeader then
											countOr = countOr + 1
										end
									end
									...
									local factionID = exports.factions:getFactionIDFromName(checkFactionName)
									if factionID then
										local isMember, rank, isLeader = exports.factions:isPlayerInFaction(thePlayer, factionID)
										if isMember and isLeader then
											countOr = countOr + 1
										end
									end
