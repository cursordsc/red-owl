function createItem(id, itemID, itemValue, ...)
	...
	if o then
		exports["anticheat"]:changeProtectedElementDataEx(o, "id", id)
		exports["anticheat"]:changeProtectedElementDataEx(o, "itemID", itemID)
		exports["anticheat"]:changeProtectedElementDataEx(o, "itemValue", itemValue, itemValue ~= 1)
...
function updateItemValue(element, newValue)
	if getElementParent(getElementParent(element)) == getResourceRootElement(getThisResource()) then
		...
		if dbExec(mysql:getConnection(),"UPDATE `worlditems` SET `itemvalue`='"..(tostring(newValue)).."' WHERE `id`='"..(tostring(id)).."'") then
			exports["anticheat"]:changeProtectedElementDataEx(element, "itemValue", newValue)
			return true
		end
	end
	return false
		...
		dbQuery(
			function(qh)
				...
				if rows > 0 then
					...
					if result then
						exports["anticheat"]:changeProtectedElementDataEx(element, "worlditemData."..tostring(key), value)
						return true
					end
				else
		...
		function(qh)
			...
			if rows > 0 then
				for index, row in ipairs(res) do
					...
					exports["anticheat"]:changeProtectedElementDataEx(element, "worlditemData."..tostring(key), value)
					exports["anticheat"]:changeProtectedElementDataEx(element, "worlditems.loaded.data."..tostring(key), true)
				end
			end
		end,
		...
		function(qh, element)
			...
			if rows > 0 then
				for index, row in ipairs(res) do
					...
					if element then
						exports["anticheat"]:changeProtectedElementDataEx(element, "worlditemData."..tostring(row.key), row.value)
					end
				end
			end
		end,
...
function setPermissions(element, permissions)
	if getElementParent(getElementParent(element)) == getResourceRootElement(getThisResource()) then
		...
		if result then
			exports["anticheat"]:changeProtectedElementDataEx(element, "worlditem.permissions", permissions)
			return true
		end
	end
	return false
