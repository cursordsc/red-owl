				...
				function(qh)
					...
					if rows > 0 then
						for index, row in ipairs(res) do
							if row then
								local result4 = dbExec(mysql:getConnection(),"UPDATE `interiors` SET `interiorx`='".. row["posX"] .."', `interiory`='".. row["posY"] .."', `interiorz`='".. row["posZ"] .."', `interior`='".. row["interior"] .."' WHERE `id`='".. tostring(dimension) .."'")	
