function lockUnlockInside(vehicle)
	...
	if  seat > 0 then
		outputChatBox("#ebc746San Andreas:#ffffff Aracın şoför koltuğunda olmanız gerekli",source,255,100,100,true)
	return end
	if ( getElementData(vehicle, "Impounded") or 0 ) == 0 then
		if not exports.global:hasItem( source, 3, dbid ) then
...
function karakterDegistirmeEvent()
	exports.global:sendLocalText(source, "- "..getPlayerName(source):gsub("_", " ").." adlı oyuncu karakter değiştirme ekranına gitme isteği gönderdi. (20 saniye içerisinde gidecek.)", 255, 255, 255, 10)
end
addEvent("karakterDegistirmeStart", true)
addEventHandler("karakterDegistirmeStart", root, karakterDegistirmeEvent)

function karakterDegistirmeEvent()
	exports.global:sendLocalText(source, "- "..getPlayerName(source):gsub("_", " ").." adlı oyuncu karakter değiştirme ekranına gitti. (kendi isteğiyle.)", 255, 255, 255, 10)
end
