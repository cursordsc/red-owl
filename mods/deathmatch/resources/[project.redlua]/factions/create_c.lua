function birlikKurGUI()
	...
	birlikWindow = guiCreateWindow((screenW - 288) / 2, (screenH - 321) / 2, 288, 321, "RED:LUA Scripting Roleplay - Birlik Sistemi", false)
	...
	kurBtn = guiCreateButton(10, 217, 267, 39, "Birlik Kur (50.000$)", false, birlikWindow)
...
function birlikSeviyeGUI(birlikIsmi, birlikLevel)
	local screenW, screenH = guiGetScreenSize()
	local birlikFiyat = 1500000 * birlikLevel
	seviyeWindow = guiCreateWindow((screenW - 289) / 2, (screenH - 181) / 2, 289, 230, "RED:LUA Scripting - Birlik Seviyesi ", false)
...
function adminBirlikKur()
	guiSetInputMode("no_binds_when_editing")
	local screenW, screenH = guiGetScreenSize()
	adminBirlikWindow = guiCreateWindow((screenW - 288) / 2, (screenH - 321) / 2, 288, 321, "RED:LUA Scripting Roleplay - Birlik Oluştur", false)
...
function makeFaction()
	if not exports["integration"]:isPlayerSeniorAdmin(localPlayer) then return end
	adminBirlikKur()
end
