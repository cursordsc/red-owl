	...
	if selection == 0 then -- Items
		for k,v in pairs(factionTable[key][3]) do
			if tonumber(v[2]) > 0 then
				...
				guiGridListSetItemText(DutyAllow.gridlist[1], row, 2, exports["items"]:getItemName(v[2]), false, false)
				guiGridListSetItemText(DutyAllow.gridlist[1], row, 3, exports["items"]:getItemDescription(v[2], v[3]), false, false)
...
function allowItem()
	...
	if not exports['items']:isItem(itemID) and selection == 0 then
