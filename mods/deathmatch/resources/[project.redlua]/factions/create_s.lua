...
local birlikCol = createColSphere(-2168.2783203125, 645.513671875, 1052.375, 1.5)
local birlikPickup = createPickup (-2168.2783203125, 645.513671875, 1052.375, 3, 1314, 1)
setElementDimension(birlikCol, 88)
setElementInterior(birlikCol, 1)
setElementDimension(birlikPickup, 88)
setElementInterior(birlikPickup, 1)

function birlikKurTrigger(thePlayer, cmd)
	local playerTeam = getElementData(thePlayer, "faction")
	
	if playerTeam ~= -1 then
		outputChatBox("#575757RED:LUA Scripting: #FFFFFFZaten bir birliğiniz var!", thePlayer, 255, 0, 0, true)
		return
	end
...
function birlikSeviyeTrigger(thePlayer, cmd)
	if isElementWithinColShape(thePlayer, birlikCol) then
		...
		if birlik_seviye == 6 then
			outputChatBox("#575757RED:LUA Scripting: #FFFFFFBirliğiniz son seviyedir!", thePlayer, 255, 0, 0, true)
			return
		end
		...
		if (oyuncu_birlik) and (birlikLider > 0) then
			...
			outputChatBox("#575757RED:LUA Scripting: #FFFFFFHerhangi bir birliğin lideri değilsiniz!", thePlayer, 255, 0, 0, true)
		end
	end
end
...
function adminBirlikKur(thePlayer, birlikName, birlikType)
	...
	if string.len(birlikName) < 4 then
		outputChatBox("#575757RED:LUA Scripting: #FFFFFFBirlik ismi en az 4 karakterden oluşmalıdır!", thePlayer, 255, 0, 0, true)
		return false
	elseif string.len(birlikName) > 36 then
		outputChatBox("#575757RED:LUA Scripting: #FFFFFFBirlik ismi en fazla 36 karakterden oluşmalıdır!", 255, 0, 0, true)
		return false
	end
		...
		for index, value in ipairs(getElementsByType("team")) do
			if getElementData(value, "name") == factionName then
				outputChatBox("#575757RED:LUA Scripting: #FFFFFFMaalesef, birlik ismi kullanımda!", thePlayer, 255, 0, 0, true)
				return false
			end
		end
		...
		if theTeam then
			if dbExec(mysql:getConnection(), "INSERT INTO factions SET name='" .. tostring(factionName) .. "', bankbalance='0', type='" .. (factionType) .. "'") then
				dbQuery(
					function(qh, thePlayer, theTeam, factionName, factionType)
						...
						if rows > 0 then
							for index, value in ipairs(res) do
								...
								setElementData(theTeam, "max_interiors", 20, false, true) --Don't sync at all / Dizzy
								...
								outputChatBox("#575757RED:LUA Scripting: #FFFFFF'" .. factionName .. "' isimli birliğiniz başarıyla oluşturuldu! ID #" .. id .. ".", thePlayer, 0, 255, 0, true)
								setElementData(theTeam, "type", tonumber(factionType))
								setElementData(theTeam, "id", tonumber(id))
								setElementData(theTeam, "money", 0)
								...
								exports["global"]:sendMessageToAdmins("AdmWarn: " .. getPlayerName(thePlayer) .. " yeni birlik oluşturdu! (/makefaction) Birlik Ismi: '" .. factionName .. "' Birlik ID #" .. id)
							end
						end
					end,
				...
				outputChatBox("#575757RED:LUA Scripting: #FFFFFFBirliğinizi oluştururken bir hata meydana geldi.", thePlayer, 255, 0, 0, true)
			end
		else
			outputChatBox("#575757RED:LUA Scripting: #FFFFFF'" .. tostring(factionName) .. "' isimli birlik zaten var.", thePlayer, 255, 0, 0, true)
		end
end
...
function birlikKur(thePlayer, birlikName, birlikType)
	...
	if para >= 10000 then
		...
		local theTeam = createTeam(tostring(factionName))
		if theTeam then
			if dbExec(mysql:getConnection(), "INSERT INTO factions SET name='" .. tostring(factionName) .. "', bankbalance='0', type='" .. (factionType) .. "', BirlikSeviyesi='1'") then
				dbQuery(
					function(qh, thePlayer, theTeam, factionName, factionType)
						local res, rows, err = dbPoll(qh, 0)
						if rows > 0 then
							for index, value in ipairs(res) do
								...
								setElementData(theTeam, "note", "", false)
								exports["logs"]:dbLog(thePlayer, 4, theTeam, "MAKE FACTION")
								exports["global"]:takeMoney(thePlayer, 50000)
		...
		outputChatBox("#575757RED:LUA Scripting: #FFFFFFMaalesef, birlik kuracak paranız yok.", thePlayer, 255, 0, 0, true)
	end
end
...
function birlikSeviye(thePlayer, birlikIsmi, birlikSeviye, birlikFiyat)
	...
	if para >= birlikFiyat then
		if birlikIsmi and birlikSeviye then
			...
			local result = dbExec(mysql:getConnection(),"UPDATE factions SET BirlikSeviyesi ='" .. birlikSeviye .. "' WHERE name='" .. birlikIsmi .. "'")
			if not result then
				outputChatBox("#575757RED:LUA Scripting: #FFFFFFBirliğinizin seviyesini arttırırken bir hata meydana geldi.", thePlayer, 255, 0, 0, true)
			end
			
			if result and birlikLevelArttir then
				outputChatBox("#575757RED:LUA Scripting: #FFFFFFBirliğinizin seviyesi başarıyla arttırılmıştır!", thePlayer, 0, 255, 0, true)
				exports["global"]:takeMoney(thePlayer, birlikFiyat)
			end
		end
	else
		outputChatBox("#575757RED:LUA Scripting: #FFFFFFYeterli paranız yok.", thePlayer, 255, 0, 0, true)
	end
end
			...
			if vehOwner == playerID then
				...
				if playerBirlik then
					...
					if elementSet and query then
						exports["items"]:deleteAll(3, vehID)
						outputChatBox("#575757RED:LUA Scripting: #f0f0f0Aracınız başarıyla birliğe verilmiştir!", thePlayer, 0, 255, 0, true)
					end
				else
					outputChatBox("#575757RED:LUA Scripting: #f0f0f0Bir birlikte değilsiniz.", thePlayer, 255, 0, 0, true)
				end
			else
				outputChatBox("#575757RED:LUA Scripting: #f0f0f0Araç size ait değil.", thePlayer, 255, 0, 0, true)
			end
		else
			outputChatBox("#575757RED:LUA Scripting: #f0f0f0Araç zaten bir birliğe ait.", thePlayer, 255, 0, 0, true)
		end
	else
		outputChatBox("#575757RED:LUA Scripting: #f0f0f0/"..cmd.." [Araç ID]", thePlayer, 255, 0, 0, true)
	end
end
		...
		if playerBirlik then
			if playerBirlik == vehFaction then
				if factionLeader == 1 then
					...
					if elementSet and query then
						outputChatBox("#575757RED:LUA Scripting: #f0f0f0Aracınız başarıyla sahibine geri verilmiştir!", thePlayer, 0, 255, 0, true)
					end
				else
					outputChatBox("#575757RED:LUA Scripting: #f0f0f0Aracı sahibine geri verebilmek için birlik lideri olmalısınız.", thePlayer, 255, 0, 0, true)
				end
			end
		else
			outputChatBox("#575757RED:LUA Scripting: #f0f0f0Bir birlikte değilsiniz.", thePlayer, 255, 0, 0, true)
