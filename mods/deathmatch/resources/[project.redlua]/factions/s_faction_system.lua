			...
			Async:foreach(res,
				function(row)
					...
					if theTeam then
						...
						exports["anticheat"]:changeProtectedElementDataEx(theTeam, "note", row.note == "NULL" and "" or row.note, false)
						exports["anticheat"]:changeProtectedElementDataEx(theTeam, "fnote", row.fnote == "NULL" and "" or row.fnote, false)
						exports["anticheat"]:changeProtectedElementDataEx(theTeam, "phone", row.phone ~= "NULL" and row.phone or nil, false)
						exports["anticheat"]:changeProtectedElementDataEx(theTeam, "max_interiors", tonumber(row.max_interiors), false, true)
						setElementData(theTeam, "birlik_level", tonumber(row["BirlikSeviyesi"])) --Don't sync at all / Dizzy
						setElementData(theTeam, "birlik_onay", tonumber(row["BirlikOnayi"])) --Don't sync at all / Dizzy
				...
				function()
					...
					setElementData(getResourceRootElement(getResourceFromName("factions")), "factionDuty", custom)
					setElementData(getResourceRootElement(getResourceFromName("factions")), "factionLocations", locations)
	
					local players = exports["pool"]:getPoolElementsByType("player")
					for k, thePlayer in ipairs(players) do
						if getElementData(thePlayer, "loggedin") == 1 then
							setPlayerTeam(thePlayer, exports["pool"]:getElement("team", (getElementData(thePlayer, "faction") or -1)) or citteam)
						end
					end
				end
			)
	...
	if (logged==1) then
		...
		if (menuVisible==0) then
			...
			if (factionID~=-1) then
						...
						function(qh, source)
							...
							if rows > 0 then
								...
								for index, row in ipairs(res) do
									...
									if phone and row.faction_phone ~= "NULL" and tonumber(row.faction_phone) then
				...
				outputChatBox("#575757RED:LUA Scripting: #f4f4f4Herhangi bir birlikte bulunmuyorsunuz.", source, 255, 0, 0, true)
			end
		else
...
function callbackUpdateRanks(ranks, wages)
	...
	if (wages) then
		for i = 1, 20 do
			wages[i] = math.min(5000, math.max(0, tonumber(wages[i]) or 0))
		end
...
function callbackUpdateNote(note)
	...
	local theTeam = getPlayerTeam(client)
	if (factionID~=-1) then
		if dbExec(mysql:getConnection(), "UPDATE factions SET note = ? WHERE id= ? ", tostring((note)), factionID) then
...
function callbackUpdateFNote(fnote)
	...
	local theTeam = getPlayerTeam(client)
	if (factionID~=-1) then
		if dbExec(mysql:getConnection(), "UPDATE factions SET fnote = ? WHERE id= ? ", tostring((fnote)), factionID) then
...
function callbackQuitFaction()
	...
	elseif dbExec(mysql:getConnection(), "UPDATE characters SET faction_id='-1', faction_leader='0', duty = 0, faction_perks='{}' WHERE charactername='" .. safename .. "'") then
		exports["infobox"]:addBox(client, "success","'" .. theTeamName .. "' isimli birlikten çıkış yaptın.")
		exports["global"]:takeItem(client, 87)
		exports["global"]:takeItem(client, 87)
		exports["global"]:takeItem(client, 87)
		exports["global"]:takeItem(client, 86)
		exports["global"]:takeItem(client, 86)
		exports["global"]:takeItem(client, 86)
		exports["global"]:takeItem(client, 64)
		exports["global"]:takeItem(client, 64)
		exports["global"]:takeItem(client, 64)
...
-- Dizzy'nin kıymetli elleri tarafından tamamiyle düzenlendi.
...
function factionActive(thePlayer, commandName)
			...
			if takim and takim > 0 then
				local takimveri = exports["pool"]:getElement("team", takim)
				local takimisim = getTeamName(takimveri)
				local takimoyunculari = getPlayersInTeam(takimveri)
				
				if #takimoyunculari == 0 then
					outputChatBox("#575757 RED:LUA Scripting:#b9c9bf".. takimisim .."' maalesef oluşumunda hiç çevrimiçi oyuncu bulamadım.", thePlayer, 255, 255, 255, true)
				else
					...
					for k, takimoyuncular in ipairs(takimoyunculari) do
						...
						outputChatBox("#575757 RED:LUA Scripting: #b9c9bf  Karakter : #FFFFFF".. getPlayerName(takimoyuncular) .." #b9c9bf Rütbe : #FFFFFF".. oyuncurutbe .. " #b9c9bf Telefon : - #FFFFFF ", thePlayer, 255, 255, 255, true)
					end
				end
			else
				outputChatBox("#575757 RED:LUA Scripting: #b9c9bf Herhangi bir oluşumda değilsiniz, lütfen oluşuma girdikten sonra tekrar deneyiniz.", thePlayer, 255, 255, 255, true)
			end
	end
