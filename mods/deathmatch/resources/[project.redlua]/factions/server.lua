...
function dutyOffduty()
	...
	if tonumber(grantID) > 0 then
		...
		local correction = 0
		local items = exports['items']:getItems( thePlayer ) -- [] [1] = itemID [2] = itemValue
		Async:foreach(items, function(itemCheck, itemSlot)
			if (itemCheck[1] == 115) then -- Weapon
				...
				if (tonumber(serialNumberCheck[2]) == 2) then -- /duty spawned
					exports['items']:takeItemFromSlot(thePlayer, itemSlot - correction, false)
		...
		if savedSkin then
			...
			exports['items']:doItemGiveawayChecks(thePlayer, 16)
