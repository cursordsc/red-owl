function openDutyWindow()
	...
	if #availablePackages > 0 then
		...
		outputChatBox("#575757RED:LUA Scripting:#f9f9f9 Bu bölgede iş başı yapamazsın.", 255, 0, 0, true)
	end
end
...
function selectPackageGUI_open(availablePackages, allowList)
	...
	for pIndex, packageDetails in ipairs(availablePackages) do
		...
		for index, itemDetails in pairs(packageDetails[5]) do
			...
			gui[guiPrefix.."pushButton1"] = guiCreateButton(xAxis, yAxis, 71, 51, exports['items']:getItemName(itemDetails[2]), false, gui[guiTabName])
		...
		if btnAction then
			...
			elseif btnAction == "Go" then
				...
				if grantID then
					...
					--	outputChatBox("i: "..a[1].." (".. exports['items']:getItemName(a[1]) ..") v:"..a[2])
					--end
