...
function dbLog(logSource, actionID, affected, data)
	...
	if logSource == nil then 
		----outputDebugString("RED:LUA Scripting : Log kaydı açık değil. "..tostring(actionID))
		return false
	end
	
	local sourceStr = dbLogDetectTypeSource(logSource)
	if sourceStr == false then
		----outputDebugString("RED:LUA Scripting : Log değeri girili değil."..tostring(actionID))
		return false
	end
