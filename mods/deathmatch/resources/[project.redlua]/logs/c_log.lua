...
function openFile( )
	...
	if not xmlFile then
		...
		xmlNodeSetValue( title, ( "RED:LUA Scripting :: Client Logs :: %04d-%02d-%02d" ):format( time.year + 1900, time.month + 1, time.monthday ) )
...
function drawInfoBox()
	closeInfoBox()
    GUIEditor.window[1] = guiCreateWindow(675, 318, 361, 189, "RED:LUA Scripting Client Logging", false)
    guiWindowSetSizable(GUIEditor.window[1], false)
    exports["global"]:centerWindow(GUIEditor.window[1])
    GUIEditor.label[1] = guiCreateLabel(14, 22, 333, 130, "RED:LUA Scripting Client Logging is a feature that writes down everything of your chatbox and your cellphone conversations then stores them in folders on your PC.\n\nThey are located in your MTA installation folder under:\n ../mods/deathmatch/resources/RED:LUA ScriptingLogs\n\nThese features can also be toggled in F10 Game Settings.", false, GUIEditor.window[1])
